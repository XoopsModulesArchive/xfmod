...
$GLOBALS['xoopsOption']['template_main'] = 'forum/xfmod_forum.html';

$forum_id = $_POST['forum_id'] ?? $_GET['forum_id'];
...
if ($forum_id) {
    ...
    */
    ...
    $result = $xoopsDB->queryF(
        'SELECT group_id,forum_name,is_public,allow_anonymous,send_all_posts_to ' . 'FROM ' . $xoopsDB->prefix('xf_forum_group_list') . ' ' . "WHERE group_forum_id='$forum_id'"
    );
    ...
    if (1 != unofficial_getDBResult($result, 0, 'is_public')) {
    ...
    */
    ...
    if (!$thread_id) {
        ...
        if ($group_id == $xoopsForge['sysnews']) {
            ...
            $res = $xoopsDB->query(
                'SELECT group_forum_id,forum_name' . ' FROM ' . $xoopsDB->prefix('xf_forum_group_list') . " WHERE group_id='$group_id' AND is_public IN ($public_flag)"
            );
        ...
        $texts = [_XF_FRM_SHOW . ' 25', _XF_FRM_SHOW . ' 50', _XF_FRM_SHOW . ' 75', _XF_FRM_SHOW . ' 100'];
    ...
    if ($thread_id) {
        ...
        */
        ...
        $ret_val .= "<table border='0' width='100%'>" . "<tr class='bg2'>" . '<td><b>' . _XF_FRM_TOPIC . '</b></td>' . '<td><b>' . _XF_FRM_TOPICSTARTER . '</b></td>' . '<td><b>' . _XF_FRM_REPLIES . '</b></td>' . '<td><b>' . _XF_FRM_LASTPOST . '</b></td>' . '</tr>';
        ...
        while (($row = $xoopsDB->fetchArray($result)) && ($i < $max_rows)) {
            $ret_val .= "<TR class='"
                        ...
                        . "<img src='"
                        . XOOPS_URL
                        . "/modules/xfmod/images/ic/cfolder15.png' width='15' height='13' border='0' alt='thread'> &nbsp; ";

            /*
                    show the subject and poster
            */

            $ret_val .= $row['subject'] . '</A></TD>' . '<TD>' . $row['uname'] . '</TD>' . '<TD>' . $row['followups'] . '</TD>' . '<TD>' . date($sys_datefmt, $row['recent']) . '</TD></TR>';
    ...
    */
    ...
    $ret_val .= "<TABLE WIDTH='100%' BORDER='0'>" . "<TR BGCOLOR='#EEEEEE'><TD WIDTH='50%'>";
    ...
    if (0 != $offset) {
        $ret_val .= "<FONT face='Arial, Helvetica' SIZE='3' STYLE='text-decoration: none'>"
                    ...
                    . "<img src='"
                    . XOOPS_URL
                    . "/modules/xfmod/images/t2.gif' width='15' height='15' border='0' align='middle' alt='"
                    . _XF_FRM_PREVIOUSMESSAGES
                    . "'> "
                    . _XF_FRM_PREVIOUSMESSAGES
                    . '</A></B></FONT>';
    } else {
    ...
    if ($xoopsDB->getRowsNum($result) > $max_rows) {
        $ret_val .= "<FONT face='Arial, Helvetica' SIZE='3' STYLE='text-decoration: none'><B>"
                    . "<A HREF='"
                    . XOOPS_URL
                    . '/modules/xfmod/forum/forum.php?max_rows='
                    . $max_rows
                    . '&offset='
                    . ($offset + $i)
                    . '&forum_id='
