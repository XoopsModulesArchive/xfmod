...
$GLOBALS['xoopsOption']['template_main'] = 'forum/xfmod_message.html';
...
if ($msg_id) {
    ...
    */
    ...
    $result = $xoopsDB->query(
        'SELECT fgl.send_all_posts_to,fgl.group_id,'
    ...
    $sql = 'SELECT u.uname,f.group_forum_id,f.thread_id,f.subject,f.date,f.body ' . 'FROM ' . $xoopsDB->prefix('xf_forum') . ' f,' . $xoopsDB->prefix('users') . ' u ' . 'WHERE u.uid=f.posted_by ' . "AND f.msg_id='$msg_id';";
    ...
    $content .= "<table border='0' width='100%'>" . "<tr class='bg2'>" . '<td><b>' . _XF_G_MESSAGE . ': ' . $msg_id . '</b></td>' . '</tr>';
    ...
    $content .= _XF_G_SUBJECT . ': ' . $ts->htmlSpecialChars(htmlspecialchars(unofficial_getDBResult($result, 0, 'subject'), ENT_QUOTES | ENT_HTML5)) . '<P>';

    $content .= $ts->displayTarea(unofficial_getDBResult(htmlspecialchars($result, 0, 'body')));
