...
function forum_header($group_id, $forum_id, $forum_name, $isAdmin)
{
    ...
    */
    ...
    if ($group_id == $xoopsForge['sysnews']) {
        ...
        if ($forum_id) {
          ...
          <B>' . _XF_G_SUMMARY . ':</B><A HREF="' . XOOPS_URL . '/modules/xfmod/forum/forum.php?forum_id=' . unofficial_getDBResult($result, 0, 'forum_id') . '">' . $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'summary')) . '</A>
          <P>
          ' . $ts->displayTarea(unofficial_getDBResult($result, 0, 'details'));
    ...
    */
    ...
    if ($forum_id && $forum_name) {
        $content .= "<P><H4 style='text-align:left;'>" . _XF_FRM_DISCUSSIONFORUMS . ": <A HREF='" . XOOPS_URL . '/modules/xfmod/forum/forum.php?forum_id=' . $forum_id . "'>" . $ts->displayTarea($forum_name) . '</A></H4>';
    }
    ...
    if ($forum_id && $xoopsUser) {
        ...
        $sql = 'SELECT monitor_id FROM ' . $xoopsDB->prefix('xf_forum_monitored_forums') . " WHERE forum_id='" . $forum_id . "' AND user_id='" . $xoopsUser->uid() . "'";

        $result = $xoopsDB->query($sql);

        $content .= "<A HREF='" . XOOPS_URL . '/modules/xfmod/forum/monitor.php?forum_id=' . $forum_id . "'>" . "<img width='16' height='15' border='0' src='" . XOOPS_URL . '/modules/xfmod/images/ic/';
...
function recursiveDeleteMessage($msg_id, $forum_id)
{
    ...
    */
    ...
    if ('' == $msg_id || '0' == $msg_id || (mb_strlen($msg_id) < 1)) {
        return 0;
    }

    $sql = 'SELECT msg_id ' . 'FROM ' . $xoopsDB->prefix('xf_forum') . ' ' . "WHERE is_followup_to='$msg_id' " . "AND group_forum_id='$forum_id'";
...
function forum_link_external($group_id, $forum_name, $forum_server)
{
    ...
    $sql = 'INSERT INTO ' . $xoopsDB->prefix('xf_forum_ext_group_list') . ' (group_id,forum_name,forum_url) ' . "VALUES ('$group_id', '$forum_name', '$url')";
...
function forum_create_forum($group_id, $forum_name, $is_public = 1, $create_default_message = 1, $description = '', $verbose = 1)
{
    ...
    */
    ...
    $sql = 'INSERT INTO ' . $xoopsDB->prefix('xf_forum_group_list') . ' (group_id,forum_name,is_public,description) ' . "VALUES ('$group_id','" . $ts->addSlashes($forum_name) . "','$is_public','" . $ts->addSlashes($description) . "')";
    ...
    if ($create_default_message) {
        ...
        $result3 = $xoopsDB->queryF(
            'INSERT INTO ' . $xoopsDB->prefix('xf_forum') . ' ' . '(group_forum_id,posted_by,subject,body,date,is_followup_to,thread_id) ' . "VALUES ('$forum_id','" . $xoopsUser->getVar('uid') . "','Welcome to " . $ts->addSlashes($forum_name) . "'," . "'Welcome to " . $ts->addSlashes(
                $forum_name
            ) . "','" . time() . "','0','" . get_next_thread_id() . "')"
        );
    }
...
function forum_show_links($forum_id, $forum_name, $group_id, $isAdmin)
{
    ...
    if ($forum_id && $forum_name) {
        $content .= "<P><H4 style='text-align:left;'>" . _XF_FRM_DISCUSSIONFORUMS . ": <A HREF='" . XOOPS_URL . '/modules/xfmod/forum/forum.php?forum_id=' . $forum_id . "'>" . $ts->htmlSpecialChars($forum_name) . '</A></H4>';
    }

    $content .= '<P><B>';

    if ($forum_id && $xoopsUser) {
        $content .= "<A HREF='" . XOOPS_URL . '/modules/xfmod/forum/monitor.php?forum_id=' . $forum_id . "'>" . "<img src='" . XOOPS_URL . "/modules/xfmod/images/ic/check.png' width='16' height='15' border'0' alt='" . _XF_FRM_MONITORFORUM . "'>" . ' ' . _XF_FRM_MONITORFORUM . '</A>';
    }
...
function forum_show_a_nested_message($result)
{
    ...
    */
    ...
    $ret_val = "<TABLE BORDER='0'>"
               ...
               . "<img src='"
               . XOOPS_URL
               . "/modules/xfmod/images/msg.gif' width='10' height='12' border'0' alt='"
               . _XF_G_REPLY
               . "'>"
               . $ts->htmlSpecialChars(htmlspecialchars($result['subject'], ENT_QUOTES | ENT_HTML5))
               . ' [ '
               . _XF_G_REPLY
               . ' ]</A> &nbsp; '
               . '<BR>'
               . date($sys_datefmt, $result['date'])
               . '</TD></TR>'
               . '<TR><TD>'
               . $ts->displayTarea(htmlspecialchars($result['body'], ENT_QUOTES | ENT_HTML5))
               . '</TD>'
               . '</TR>'
               . '</TABLE>';
...
function show_thread($thread_id)
{
    ...
    */
    ...
    while (false !== ($row = $xoopsDB->fetchArray($result))) {
        $msg_arr[(string)$row[is_followup_to]][] = $row;
    }
    ...
    */
    ...
    $ret_val .= "<table border='0' width='100%'>" . "<tr class='bg2'>" . '<td><b>' . _XF_FRM_THREAD . '</b></td>' . '<td><b>' . _XF_FRM_AUTHOR . '</b></td>' . '<td><b>' . _XF_FRM_DATE . '</b></td>' . '</tr>';
    ...
    $ret_val .= "<TR class='"
                    ...
                    . "<img src='"
                    . XOOPS_URL
                    . "/modules/xfmod/images/msg.gif' width='10' height='12' border'0' alt='message'>";

    /*
        See if this message is new or not
    */

    $ret_val .= $ts->htmlSpecialChars($thread['subject']) . '</A></TD>' . '<TD>' . $thread['uname'] . '</TD>' . '<TD>' . date($sys_datefmt, $thread['date']) . '</TD></TR>';
    ...
    if ($rows > 0) {
        for ($i = ($rows - 1); $i >= 0; $i--) {
            ...
            */
            ...
            $ret_val .= (($current_message != $msg_arr[$msg_id][$i]['msg_id']) ? "<A HREF='" . XOOPS_URL . '/modules/xfmod/forum/message.php?msg_id=' . $msg_arr[$msg_id][$i]['msg_id'] . "'>" : '')
                        . "<img src='"
                        . XOOPS_URL
                        . "/modules/xfmod/images/msg.gif' width='10' height='12' border'0' alt='message'>";
            ...
            */
            ...
            $ret_val .= $ts->htmlSpecialChars($msg_arr[$msg_id][$i]['subject']) . '</A></TD>' . '<TD>' . $msg_arr[$msg_id][$i]['uname'] . '</TD>' . '<TD>' . date($sys_datefmt, $msg_arr[$msg_id][$i]['date']) . '</TD></TR>';
 ...
 * @param mixed $is_followup_to
 * @param mixed $subject
 * @param mixed $body
 * @param mixed $group_forum_id
 * @return bool
 * @return bool
 */
function post_message($thread_id, $is_followup_to, $subject, $body, $group_forum_id)
{
    ...
    if (!$thread_id) {
        ...
        if ($is_followup_to) {
            ...
            if ($xoopsDB->getRowsNum($res2) > 0) {
                ...
                $res4 = $xoopsDB->queryF(
                    'UPDATE ' . $xoopsDB->prefix('xf_forum') . " SET most_recent_date='" . time() . "' " . "WHERE thread_id='$thread_id' AND is_followup_to='0'"
                );
                ...
                $res3 = $xoopsDB->queryF(
                        'UPDATE ' . $xoopsDB->prefix('xf_forum') . " SET has_followups='1',most_recent_date='" . time() . "' " . "WHERE msg_id='$is_followup_to'"
                    );
    ...
    $sql = 'INSERT INTO '
              ...
              . "VALUES ('$group_forum_id', '"
              . $xoopsUser->getVar('uid')
              . "', '"
              . $ts->addSlashes($subject)
              . "', '"
              . $ts->addSlashes($body)
              . "', '"
              . time()
              . "','$is_followup_to','$thread_id','"
              . time()
              . "')";
...
function show_post_form($forum_id, $thread_id = 0, $is_followup_to = 0, $subject = '')
{
    ...
    if (!$xoopsUser) {
        ...
        $content .= "<INPUT TYPE='TEXT' NAME='subject' VALUE='" . $ts->htmlSpecialChars($subject) . "' SIZE='45' MAXLENGTH='45'>";
...
function handle_monitoring($forum_id, $msg_id)
{
    ...
    */
    ...
    $sql = 'SELECT u.email ' . 'FROM ' . $xoopsDB->prefix('xf_forum_monitored_forums') . ' fmf,' . $xoopsDB->prefix('users') . ' u ' . "WHERE fmf.user_id=u.uid AND fmf.forum_id='$forum_id'";
    ...
    if (($result && $rows > 0) || $send_all_posts_to) {
        ...
        if ($send_all_posts_to) {
            $bcc_arr[count($bcc_arr)] = $send_all_posts_to;
        }
        ...
        if ($result && $xoopsDB->getRowsNum($result) > 0) {
            ...
            $message = frmGetMonitorMessage($msg_id, $uname, $ts->htmlSpecialChars($body), $forum_id, $unix_group_name, $ts->htmlSpecialChars($forum_name), $ts->htmlSpecialChars($subject));

            $r = xoopsForgeMail($xoopsForge['noreply'], $xoopsConfig['sitename'], $message['subject'], $message['body'], [$xoopsForge['noreply']], $bcc_arr);
