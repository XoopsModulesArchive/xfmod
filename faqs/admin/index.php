...
$GLOBALS['xoopsOption']['template_main'] = 'faqs/admin/xfmod_index.html';
...
if ($post_changes) {
    ...
    */
    ...
    if ($delete_faq) {
        $sql = 'DELETE FROM ' . $xoopsDB->prefix('xf_foundry_faqs') . " WHERE foundry_id='" . $group_id . "'" . " AND category_id='" . $cat_id . "'";
    ...
    } elseif ($delete_contents) {
        $sql = 'DELETE FROM ' . $xoopsDB->prefix('xoopsfaq_contents') . " WHERE contents_id='" . $delete_contents . "'";
        ...
        $sql = 'INSERT INTO ' . $xoopsDB->prefix('xoopsfaq_categories') . " (category_title) VALUES ('" . $faq_title . "')";
        ...
        if (!$result) {
            ...
            $sql = 'INSERT INTO ' . $xoopsDB->prefix('xf_foundry_faqs') . ' (foundry_id, category_id) VALUES ' . "('" . $group_id . "', '" . $insert_id . "')";
        ...
        $sql = 'INSERT INTO ' . $xoopsDB->prefix('xf_foundry_faqs') . ' (foundry_id, category_id) VALUES ' . "('" . $group_id . "', '" . $cat_id . "')";
        ...
        $sql = 'INSERT INTO ' . $xoopsDB->prefix('xoopsfaq_contents') . ' (category_id,contents_title,contents_contents,contents_time)' . " VALUES ('" . $cat_id . "', '" . $question . "', '" . $answer . "', CURTIME())";
...
if ($add_faq) {
    ...
    */
    ...
    $sql = 'SELECT category_title,category_id FROM ' . $xoopsDB->prefix('xoopsfaq_categories');

    $result = $xoopsDB->query($sql);

    while (false !== ($catrow = $xoopsDB->fetchArray($result))) {
        $sql = 'SELECT * FROM ' . $xoopsDB->prefix('xf_foundry_faqs') . " WHERE foundry_id='" . $group_id . "'" . " AND category_id='" . $catrow['category_id'] . "'";
    ...
    */
    ...
    $sql = 'SELECT contents_title, contents_contents FROM ' . $xoopsDB->prefix('xoopsfaq_contents') . " WHERE contents_id='" . $contents_id . "'";
    ...
    */
    ...
    $sql = 'SELECT ff.category_id, xf.category_title ' . 'FROM ' . $xoopsDB->prefix('xf_foundry_faqs') . ' ff, ' . $xoopsDB->prefix('xoopsfaq_categories') . ' xf ' . "WHERE ff.foundry_id='" . $group_id . "' " . 'AND xf.category_id=ff.category_id';
    ...
    while (false !== ($row = $xoopsDB->fetchArray($result))) {
        ...
        $sql = 'SELECT contents_id, contents_title' . ' FROM ' . $xoopsDB->prefix('xoopsfaq_contents') . " WHERE category_id='" . $cat_id . "'" . ' AND contents_visible=1';

        $qaresult = $xoopsDB->query($sql);

        if ($xoopsDB->getRowsNum($qaresult) > 0) {
            while (false !== ($row = $xoopsDB->fetchArray($qaresult))) {
                $content .= "<tr><td colspan='2'>&nbsp; &nbsp;<a href='"
                            . XOOPS_URL
                            . '/modules/xoopsfaq/?cat_id='
                            . $cat_id
                            . '&contents_id='
