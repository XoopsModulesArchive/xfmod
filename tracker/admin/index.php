...
require XOOPS_ROOT_PATH . '/modules/xfmod/language/english/tracker_text_1.php';
$GLOBALS['xoopsOption']['template_main'] = 'tracker/admin/xfmod_index.html';
...
if ($group_id && $atid) {
        ...
        if ($add_cat) {
            ...
            if ($add_all) {
                $sql = 'SELECT u.uid '
                       ...
                       . " ap ON ap.user_id=u.uid AND ap.group_artifact_id='$atid' "
    ...
    if ($add_cat) {
        ...
        */
        ...
        if ($result && $rows > 0) {
            $content .= "<TABLE cellspacing='1' cellpadding='5' width='100%' border='0'>" . "<TR class='bg2'>" . "<TD align='center'><b>" . _XF_TRK_ID . '</b></td>' . "<TD align='center'><b>" . _XF_TRK_TITLE . '</b></td>' . '</TR>';
            ...
            for ($i = 0; $i < $rows; $i++) {
                $content .= "<TR class='"
                            ...
                            . $ath->getID()
                            . "'>"
                            . $ts->htmlSpecialChars(unofficial_getDBResult($result, $i, 'category_name'))
                            . '</A></TD></TR>';
            }
        ...
        */
        ...
        if ($result && $rows > 0) {
            ...
            for ($i = 0; $i < $rows; $i++) {
                $content .= "<TR class='"
                            ...
                            . $ath->getID()
                            . "'>"
                            . $ts->htmlSpecialChars(unofficial_getDBResult($result, $i, 'title'))
                            . '</A></TD></TR>';
            }
        ...
        $sql = 'SELECT ap.id,ap.group_artifact_id,ap.user_id,ap.perm_level,u.uname,u.name ' . 'FROM ' . $xoopsDB->prefix('xf_artifact_perm') . ' ap, ' . $xoopsDB->prefix('users') . ' u ' . 'WHERE u.uid=ap.user_id ' . "AND ap.group_artifact_id='" . $ath->getID() . "'";
        ...
        if (!$res || $xoopsDB->getRowsNum($res) < 1) {
            ...
            $content .= "<TABLE cellspacing='1' cellpadding='5' width='100%' border='0'>" . "<TR class='bg2'>" . "<TD align='center'><b>" . _XF_G_DELETE . '</b></td>' . "<TD align='center'><b>" . _XF_TRK_USERNAME . '</b></td>' . "<TD align='center'><b>" . _XF_TRK_PERMISSION . '</b></td>' . '</TR>';
            ...
            while (false !== ($row_dev = $xoopsDB->fetchArray($res))) {
                $content .= '
				...
				<OPTION value="0"' . ((0 == $row_dev['perm_level']) ? ' selected' : '') . '>-
				<OPTION value="1"' . ((1 == $row_dev['perm_level']) ? ' selected' : '') . '>' . _XF_TRK_TECHNICIAN . '
				<OPTION value="2"' . ((2 == $row_dev['perm_level']) ? ' selected' : '') . '>' . _XF_TRK_ADMINTECH . '
				<OPTION value="3"' . ((3 == $row_dev['perm_level']) ? ' selected' : '') . '>' . _XF_TRK_ADMINONLY . '
				</SELECT></FONT></TD>
        ...
        $content .= html_build_multiple_select_box($res, 'addids[]', [], 8, false);
        ...
        if (!$acr || !is_object($acr)) {
			...
			<b>' . _XF_TRK_TITLE . ':</b><BR>
			<INPUT TYPE="TEXT" NAME="title" VALUE="' . $ts->htmlSpecialChars($acr->getTitle()) . '" SIZE="50" MAXLENGTH="50">
			<P>
			<B>' . _XF_TRK_MESSAGEBODY . ':</B><BR>
			<TEXTAREA NAME="body" ROWS="30" COLS="65" WRAP="HARD">' . $ts->htmlSpecialChars($acr->getBody()) . '</TEXTAREA>
			<P>
			<INPUT TYPE="SUBMIT" NAME="post_changes" VALUE="' . _XF_G_SUBMIT . '">
			</FORM>';
        }
        ...
        */
        ...
        if (!$ac || !is_object($ac)) {
			...
			<INPUT TYPE="TEXT" NAME="name" VALUE="' . $ts->htmlSpecialChars($ac->getName()) . '">
			<P>
			<B><' . _XF_TRK_AUTOASSIGNTO . ':</B><BR>
    ...
    //$ath->footer();
        ...
        */
        ...
        if (!$ag || !is_object($ag)) {
			...
			<INPUT TYPE="TEXT" NAME="name" VALUE="' . $ts->htmlSpecialChars($ag->getName()) . '">
			<P>
			<INPUT TYPE="SUBMIT" NAME="post_changes" VALUE="' . _XF_G_SUBMIT . '">
			</FORM>';
        }
        ...
        if ($ath->getDataType()) {
            ...
            $content .= '<INPUT TYPE="TEXT" NAME="name" VALUE="' . $ts->htmlSpecialChars($ath->getName()) . '">';
        }
        ...
        if ($ath->getDataType()) {
            ...
            $content .= '<INPUT TYPE="TEXT" NAME="description" VALUE="' . $ts->htmlSpecialChars($ath->getDescription()) . '" SIZE="50">';
        }
		...
		<TEXTAREA NAME="submit_instructions" ROWS="10" COLS="55" WRAP="HARD">' . $ts->htmlSpecialChars($ath->getSubmitInstructions()) . '</TEXTAREA>
		<P>
		<B>' . _XF_TRK_FREEFORMBROWSE . ':</B><BR>
		<TEXTAREA NAME="browse_instructions" ROWS="10" COLS="55" WRAP="HARD">' . $ts->htmlSpecialChars($ath->getBrowseInstructions()) . '</TEXTAREA>
		<P>
		<INPUT TYPE="SUBMIT" NAME="post_changes" VALUE="' . _XF_G_SUBMIT . '">
		</FORM>';
    ...
    if (!$result || $rows < 1) {
        ...
        */
        ...
        for ($j = 0; $j < $rows; $j++) {
			...
			<A HREF="'
                        . XOOPS_URL
                        . '/modules/xfmod/tracker/admin/?atid='
                        . unofficial_getDBResult($result, $j, 'group_artifact_id')
                        . '&group_id='
                        . $group_id
                        . '"> <img src="'
                        . XOOPS_URL
                        . '/modules/xfmod/images/ic/index.png" width="24" height="24" BORDER="0" alt="index"> &nbsp;'
                        . $ts->htmlSpecialChars(unofficial_getDBResult($result, $j, 'name'))
                        . '</A><BR>'
                        . $ts->displayTarea(unofficial_getDBResult($result, $j, 'description'))
                        . '<P>';
        }
    }
