...
if ($group_id && $atid) {
    ...
    if (!$ath || !is_object($ath)) {
        redirect_header(Request::getString('HTTP_REFERER', '', 'SERVER'), 2, 'ERROR<br>ArtifactType could not be created');

        exit;
    }

    if ($ath->isError()) {
        redirect_header(Request::getString('HTTP_REFERER', '', 'SERVER'), 2, 'ERROR<br>' . $ath->getErrorMessage());
    ...
    switch ($func) {
        case 'add':
        {
            require XOOPS_ROOT_PATH . '/modules/xfmod/tracker/add.php';
            break;
        }
        case 'postadd':
        {
            ...
            */
            ...
            if (!$ah || !is_object($ah)) {
                ...
                if (!$ah->create($category_id, $artifact_group_id, $summary, $details, $assigned_to, $priority, $user_email)) {
                    ...
                    require XOOPS_ROOT_PATH . '/modules/xfmod/tracker/browse.php';
        ...
        case 'massupdate':
        {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if (!$ah || !is_object($ah)) {
                    ...
                    $_priority = ((100 != $priority) ? $priority : $ah->getPriority());

                    $_status_id = ((100 != $status_id) ? $status_id : $ah->getStatusID());

                    $_category_id = ((100 != $category_id) ? $category_id : $ah->getCategoryID());

                    $_artifact_group_id = ((100 != $artifact_group_id) ? $artifact_group_id : $ah->getArtifactGroupID());

                    $_resolution_id = ((100 != $resolution_id) ? $resolution_id : $ah->getResolutionID());

                    $_assigned_to = ((100 != $assigned_to) ? $assigned_to : $ah->getAssignedTo());
        ...
        case 'detail':
        {
            ...
            //exit;
                ...
                if ($ath->userIsAdmin() || ($xoopsUser && ($ah->getSubmittedBy() == $xoopsUser->getVar('uid')))) {
                    require XOOPS_ROOT_PATH . '/modules/xfmod/tracker/mod.php';
                } else {
                    require XOOPS_ROOT_PATH . '/modules/xfmod/tracker/detail.php';
                }
            }
            break;
        }
    ...
    $GLOBALS['xoopsOption']['template_main'] = 'tracker/xfmod_index.html';
