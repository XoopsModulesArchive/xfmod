 ...
 */
$GLOBALS['xoopsOption']['template_main'] = 'tracker/xfmod_mod.html';
...
$content = '<H4>[ #' . $ah->getID() . ' ] ' . $ts->htmlSpecialChars($ah->getSummary()) . '</H4>';

$sql = 'SELECT id FROM ' . $xoopsDB->prefix('xf_artifact_monitor') . " WHERE artifact_id='" . $aid . "' AND user_id='" . $xoopsUser->uid() . "'";
$result = $xoopsDB->query($sql);
$content .= "<P><B><A HREF='" . $_SERVER['PHP_SELF'] . '?group_id=' . $group_id . '&atid=' . $ath->getID() . '&func=monitor&artifact_id=' . $ah->getID() . "'>" . "<img width='16' height='15' border'0' src='" . XOOPS_URL . '/modules/xfmod/images/ic/';
...
if (2 == $ah->getStatusID() && $close_date > 1) {
    $content .= '<BR><B>' . _XF_TRK_CLOSEDATE . ':</B><BR>' . date($sys_datefmt, $close_date);
}
...
$content .= '
	...
	<TR>
		...
		<INPUT TYPE="TEXT" NAME="summary" SIZE="45" VALUE="' . $ts->htmlSpecialChars($ah->getSummary()) . '">
		</TD>
	</TR>

	<TR><TD COLSPAN="2">
		' . $ts->displayTarea($ah->getDetails()) . '
	</TD></TR>
...
if ($count > 0) {
    for ($i = 0; $i < $count; $i++) {
        $content .= '<TR><TD><INPUT TYPE="CHECKBOX" NAME="delete_file[]" VALUE="' . $file_list[$i]->getID() . '"> Delete</TD>' . '<TD>' . $file_list[$i]->getName() . '</TD>
				<TD>' . $ts->displayTarea($file_list[$i]->getDescription()) . '</TD>
