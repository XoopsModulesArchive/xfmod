...
function docman_header($project, $group_id, $pagehead, $style = 'xyz')
{
    ...
    $header .= "<b><a href='" . XOOPS_URL . '/modules/xfmod/docman/index.php?group_id=' . $group_id . "'>" . _XF_DOC_VIEWDOC . '</a>' . " | <a href='" . XOOPS_URL . '/modules/xfmod/docman/new.php?group_id=' . $group_id . "'>" . _XF_DOC_SUBMITNEWDOC . '</a></b>';
...
function display_groups_option($group_id = false, $checkedval = 'xzxz')
{
    ...
    $query = 'SELECT doc_group, groupname ' . 'FROM ' . $xoopsDB->prefix('xf_doc_groups') . ' ' . "WHERE group_id='$group_id' " . 'ORDER BY groupname';
...
function display_groups($group_id)
{
    ...
    $query = 'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_doc_groups') . ' ' . "WHERE group_id='$group_id'";
    ...
    if ($xoopsDB->getRowsNum($result) < 1) {
        ...
        $content .= "<table border='0' width='100%'>" . "<tr class='bg2'>" . '<td><b>' . _XF_DOC_GROUPID . '</b></td>' . '<td><b>' . _XF_DOC_GROUPNAME . '</b></td>' . '<td><b>' . _XF_DOC_CONTROLS . '</b></td>' . '</tr>';
        ...
        while (false !== ($row = $xoopsDB->fetchArray($result))) {
            $content .= "<tr class='"
                        ...
                        . "<td>[ <a href='"
                        . XOOPS_URL
                        . '/modules/xfmod/docman/admin/index.php?mode=groupdelete&doc_group='
                        . $row['doc_group']
                        . '&group_id='
                        . $group_id
                        . "'>"
                        ...
                        . "</A> ] [ <a href='"
                        . XOOPS_URL
                        . '/modules/xfmod/docman/admin/index.php?mode=groupedit&doc_group='
                        . $row['doc_group']
                        . '&group_id='
                        . $group_id
                        . "'>"
...
function display_doc_feedback($group_id, $docid, $limit)
{
    ...
    if ($xoopsDB->getRowsNum($res_feedback) > 0) {
        $query = 'SELECT uname,answer,suggestion ' . 'FROM ' . $xoopsDB->prefix('users') . ' u,' . $xoopsDB->prefix('xf_doc_feedback') . ' df ' . 'WHERE u.uid=df.user_id ' . "AND df.docid='" . $docid . "' " . 'ORDER BY entered DESC';
        ...
        $content .= '<tr>' . "<td colspan='3'>[ <a href='" . XOOPS_URL . '/modules/xfmod/docman/admin/index.php?mode=showfeedback&docid=' . $row['docid'] . '&group_id=' . $group_id . "'>" . _XF_DOC_SHOWALL . '</A> ]</td>' . '</tr></table>';
    } else {
...
function get_group_count($group_id)
{
    ...
    $query = 'SELECT COUNT(*) AS count ' . 'FROM ' . $xoopsDB->prefix('xf_doc_groups') . ' ' . "WHERE group_id='$group_id'";
...
function display_docs($style, $group_id)
{
    ...
    if ($xoopsDB->getRowsNum($result) < 1) {
        $query = 'SELECT name ' . 'FROM ' . $xoopsDB->prefix('xf_doc_states') . ' ' . "WHERE stateid='$style'";
        ...
        $content .= "<table border='0' width='100%'>" . "<tr class='bg2'>" . '<td><b>' . _XF_DOC_NAME . '</b></td>' . '<td><b>' . _XF_DOC_DOCUMENTID . '</b></td>';

        if (2 == $style) {
            $content .= '<td><b>' . _XF_DOC_DELETEDBY . '</b></td>';
        }

        $content .= '<td><b>' . _XF_DOC_UPDATEDATE . '</b></td>' . '<td><b>' . _XF_DOC_CREATEDATE . '</b></td>' . '</tr>';
        ...
        while (false !== ($row = $xoopsDB->fetchArray($result))) {
            ...
            if (2 != $style) {
                $content .= "<a href='" . XOOPS_URL . '/modules/xfmod/docman/admin/index.php?docid=' . $row['docid'] . '&mode=docedit&group_id=' . $group_id . "'>";
            }
