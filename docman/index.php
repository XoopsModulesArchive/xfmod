...
$GLOBALS['xoopsOption']['template_main'] = 'docman/xfmod_index.html';
...
if ($group_id) {
    ...
    $query = 'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_doc_groups') . ' ' . "WHERE group_id='$group_id' " . 'ORDER BY groupname';
    ...
    if ($xoopsDB->getRowsNum($result) < 1) {
        ...
        while (false !== ($row = $xoopsDB->fetchArray($result))) {
            $query = 'SELECT description, docid, title, data, doc_group, createdate, created_by, stateid ' . 'FROM ' . $xoopsDB->prefix('xf_doc_data') . " WHERE doc_group='" . $row['doc_group'] . "'" . " AND (stateid='1'";
            ...
            if ($xoopsDB->getRowsNum($subresult) > 0) {
                while (false !== ($subrow = $xoopsDB->fetchArray($subresult))) {
                    $private_doc = (1 == $subrow['stateid']) ? false : true;

                    $url = 'index.php?group_id=' . $group_id . '&docid=' . $subrow['docid'];

                    if ($private_project || $private_doc) {
                        $url .= '&private=1';
                    }
                    ...
                    $content .= '<li>' . "<b><a href='$url'>" . $ts->htmlSpecialChars($subrow['title']) . '</a></b>' . '<BR>&nbsp;&nbsp;<i>' . _XF_G_DESCRIPTION . ':</i> ' . $ts->htmlSpecialChars($subrow['description']) . '<BR>&nbsp;&nbsp;<i>' . _XF_DOC_CREATEDBY . ':</i> ' . $name;
