...
$GLOBALS['xoopsOption']['template_main'] = 'docman/xfmod_new.html';
...
if ($group_id) {
    ...
    if ('add' == $op) {
        ...
        if ($_FILES['file1']['tmp_name'] && 'none' != $_FILES['file1']['tmp_name'] && $_FILES['file1']['size'] > 0) {
        ...
        $query = 'INSERT INTO ' . $xoopsDB->prefix('xf_doc_data') . ' ' . '(stateid,title,data,createdate,updatedate,created_by,doc_group,description) ' . "VALUES ('3'," // state = 3 == pending
                 . "'" . $ts->addSlashes($doc_name) . "'," . "'" . $filename . "'," //."'".$ts->addSlashes($data)."',"
        ...
        if (!$res && $file_url) {
            ...
            $sql = 'SELECT email FROM' . ' ' . $xoopsDB->prefix('users') . ' AS u' . ',' . $xoopsDB->prefix('xf_user_group') . ' AS ug' . ' WHERE u.uid=ug.user_id' . " AND group_id=$group_id" . ' AND ug.doc_flags=1';
