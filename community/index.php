...
$GLOBALS['xoopsOption']['template_main'] = 'community/xfmod_index.html';
...
if (!isset($group_id) || !$group_id || !is_numeric($group_id)) {
    $unixname = mb_strtolower($QUERY_STRING);
...
if ($community->isInactive() && 'y' == $activate) {
    $sql = 'UPDATE ' . $xoopsDB->prefix('xf_groups') . " SET status='A', is_='1'" . " WHERE group_id='" . $group_id . "'";
...
if ($community->isInactive() && 'y' != $activate) {
    $xoopsTpl->assign(
        ...
        . _XF_COMM_THISCOMMISINACTIVE
        . "</b></td></tr>\n"
        . '<form action="'
        . XOOPS_URL
        . '/modules/xfmod/community/?'
        . $community->getUnixName()
        . "\" method=\"POST\">\n"
        . "<input type=\"hidden\" name=\"activate\" value=\"y\">\n"
        . '<tr><td align="center"><input type="submit" name="submit" value="'
        . _XF_COMM_REACTIVATECOMM
        . "\">\n"
        . "</td></tr></form></table><hr>\n"
    );
}
...
if ($community->getDescription()) {
    $description = '<P>' . $ts->displayTarea($community->getDescription()) . '<br><br><br>';
} else {
...
$res_admin = $xoopsDB->query(
    'SELECT u.uid,u.uname ' . 'FROM ' . $xoopsDB->prefix('users') . ' u,' . $xoopsDB->prefix('xf_user_group') . ' ug ' . 'WHERE ug.user_id=u.uid ' . "AND ug.group_id=$group_id " . "AND ug.admin_flags='A' " . 'AND u.level>0'
);
if ($res_admin && ($xoopsDB->getRowsNum($res_admin) > 0)) {
    ...
    while (false !== ($row_admin = $xoopsDB->fetchArray($res_admin))) {
...
if ($community->hasFeaturedProject()) {
    ...
    $pick_entry = mt_rand() % $featured_count + 1;

    $pick = 1;

    while (false !== ($featured = $community->db->fetchArray($results))) {
        if ($pick == $pick_entry) {
...
if ($community->usesDocman()) {
    ...
    $sql = 'SELECT doc_group' . ' FROM ' . $xoopsDB->prefix('xf_doc_groups') . " WHERE groupname='" . _XF_COMM_ARTICLES_KEY . "'" . " AND group_id='" . $group_id . "'";

    $result = $xoopsDB->query($sql);

    $article_title = _XF_COMM_FEATUREDARTICLE;

    $has_article = $xoopsDB->getRowsNum($result) > 0;
        ...
        if ($numarticles > 0) {
            while (false !== ($row = $xoopsDB->fetchArray($result))) {
                if ($document_private_state == $row['stateid']) {
                    $chkmembersql = 'SELECT user_group_id' . ' FROM ' . $xoopsDB->prefix('xf_user_group') . " WHERE user_id='" . $xoopsUser->uid() . "'" . " AND group_id='" . $group_id . "'";
                ...
                } elseif ($document_active_state != $row['stateid']) {
                    continue;
                } else {
                    ...
                    if (0 == strcasecmp('text/html', $mime_type)
                        ...
                        if (!$fp) {
                            ...
                            if ($pseudo_max > $size) {
                                ...
                                $body = mb_substr($fullbody, 0, $pseudo_max);
