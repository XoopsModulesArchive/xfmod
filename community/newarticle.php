...
$GLOBALS['xoopsOption']['template_main'] = 'community/xfmod_newarticle.html';
...
if ($group_id) {
    ...
    if ('add' == $op) {
        ...
        if (!is_dir($xoopsForge['ftp_path'] . '/' . $group->getUnixName())) {
            mkdir($xoopsForge['ftp_path'] . '/' . $project->getUnixName(), 0755);
        }

        if (!is_dir($xoopsForge['ftp_path'] . '/' . $group->getUnixName() . '/docs')) {
            mkdir($xoopsForge['ftp_path'] . '/' . $project->getUnixName() . '/docs', 0755);
        }
        ...
        $query = 'INSERT INTO ' . $xoopsDB->prefix('xf_doc_data') . ' ' . '(stateid,title,data,createdate,updatedate,created_by,doc_group,description) ' . "VALUES ('3'," // state = 3 == pending
                 . "'" . $ts->addSlashes($title) . "'," . "'" . $file_url . "'," //."'".$ts->addSlashes($data)."',"
                 . "'" . time() . "'," . "'" . time() . "'," . "'" . $user_id . "'," . "'" . $doc_group . "'," . "'" . $ts->addSlashes($description) . "')";
        ...
        if (!$res) {
            ...
            $content .= '<p><b>' . _XF_DOC_THANKYOUONSUBMISSION . '</b>' . "<p><a href='" . XOOPS_URL . '/modules/xfmod/docman/index.php?group_id=' . $group_id . "'>" . _XF_G_BACK . '</a>';
        }
