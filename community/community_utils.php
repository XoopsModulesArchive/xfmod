...
function get_recent_discussions($group_id)
{
    ...
    if ('newsportal' == $xoopsForge['forum_type']) {
...
require_once '../newsportal/config.inc';
require_once "../newsportal/$file_newsportal";
...
function get_nntp_discussions($group_id)
{
    ...
    if (count($feeds) > 0) {
        ...
        $discussion_content .= "<td><a href='rssfeed.php?group_id=$group_id'>" . "<img src='../newsportal/$imgdir/xml.gif' border=0 align='right' width=36 height=14 alt='XML'></a>" . '&nbsp;<b>' . _XF_COMM_TOPIC . '</b></td>' . "<td align='center'><b>" . _XF_COMM_LASTUPDATED . '</b></td>';
        ...
        foreach ($feeds as $arr) {
            ...
            $discussion_content .= '</a>&nbsp; &nbsp; &nbsp;<i> (' . "<a href='" . XOOPS_URL . '/modules/xfmod/project/?' . $arr['unix_group_name'] . "'>" . $arr['group_name'] . '</a>)</i></td>'; //&nbsp; &nbsp; &nbsp<i>".$arr["body"]."</i>
...
function send_rss_feed($group_id)
{
    ...
    [$project_short_name, $project_name, $project_desc] = $xoopsDB->fetchRow($result);
    ...
    echo '<description>' . htmlspecialchars($project_desc, ENT_QUOTES | ENT_HTML5) . '</description>';
    ...
    $valid_tags = ['title', 'description', 'author', 'link', 'pubDate', 'comments'];
    ...
    if (count($feeds) > 0) {
        foreach ($feeds as $feed) {
            ...
            foreach ($feed as $key => $value) {
                if (in_array($key, $valid_tags, true)) {
                    if ('pubDate' == $key) {
                        $value = date('r', $value);
                    }

                    echo "<$key>" . htmlspecialchars(utf8_encode($value), ENT_QUOTES | ENT_HTML5) . "</$key>";
                }
            }
...
function get_forum_discussions($group_id)
{
    ...
    $query = 'SELECT DISTINCT f.msg_id, fg.group_forum_id, fg.group_id, f.thread_id, f.posted_by, f.subject, f.body, f.date, f.most_recent_date '
             ...
             . "WHERE (fg.group_id='"
             . $group_id
             . "' AND fg.group_forum_id = f.group_forum_id)"
             . "OR  (gl.trove_cat_id='"
             . $group_id
             . "' AND gl.group_id=fg.group_id AND fg.group_forum_id = f.group_forum_id)"
