...
$GLOBALS['xoopsOption']['template_main'] = 'community/admin/xfmod_proj_list.html';
...
if ($perm->isAdmin() && isset($func)) {
    ...
    if ('disassoc' == $func) {
        $results = $xoopsDB->queryF(
            'DELETE FROM ' . $xoopsDB->prefix('xf_trove_group_link') . " WHERE trove_cat_id=$group_id" . " AND group_id=$proj_id"
        );

        if (!$results) {
            $xoopsForgeErrorHandler->addError(
                "Failed to delete association to project $proj_name: " . $xoopsDB->error()
            );
        } else {
...
$sql = 'SELECT p.group_id,p.group_name,' . 'p.short_description,p.unix_group_name FROM ' . $xoopsDB->prefix('xf_groups') . ' AS p,' . $xoopsDB->prefix('xf_trove_group_link') . ' AS c' . " WHERE c.trove_cat_id=$group_id" . ' AND p.group_id=c.group_id';
...
for ($i = 0; $i < $rowcount; $i++) {
    ...
    $content .= "<tr class='"
                ...
                . '</a></td>'
                . '<td>'
                . strip_tags(mb_substr($row['short_description'], 0, 255))
                . '...</td>'
                . "<td align='center'>"
                . "<form action='"
                . XOOPS_URL
                . '/modules/xfmod/community/admin/proj_list.php?'
                ...
                . "<input type='hidden' name='proj_name' value='"
                . $row['unix_group_name']
                . "'>"
    ...
    if (!$group->isFeaturedProject($proj_id)) {
        ...
        $content .= "<form action='"
                    ...
                    . $row['unix_group_name']
                    . "'>"
                    . "<textarea rows='6' cols='40' name='featuredescription' value=''>"
                    . strip_tags(mb_substr($row['short_description'], 0, 252))
                    . '...</textarea><BR>'
