...
$GLOBALS['xoopsOption']['template_main'] = 'community/admin/xfmod_index.html';
...
if ($perm->isAdmin() && $func) {
    ...
    if ('adduser' == $func) {
        ...
        $result = $xoopsDB->query(
            'SELECT group_id FROM ' . $xoopsDB->prefix('xf_groups') . " WHERE unix_group_name='$form_proj_name'"
        );
        ...
        if (!$result or $xoopsDB->getRowsNum($result) < 1) {
            ...
            $result = $xoopsDB->query(
                'SELECT group_id from ' . $xoopsDB->prefix('xf_trove_group_link') . " WHERE trove_cat_id=$group_id" . " AND group_id=$projid"
            );

            if ($result and $xoopsDB->getRowsNum($result) > 0) {
                $xoopsForgeErrorHandler->addError(
                    "Project $form_proj_name is" . ' already associated to this community'
                );
            } else {
                trove_setnode($projid, $group_id, $TROVE_COMMUNITY);

                $xoopsForgeErrorHandler->addMessage(
                    'Project ' . $form_proj_name . ' is now associated with this community'
                );
...
$xoopsTpl->assign(
    'misc_content',
    '<p>' . _XF_G_DESCRIPTION . ': ' . $ts->displayTarea($group->getDescription()) . '<br></p>' . '<p><b>' . _XF_COMM_TROVECATEGORIZATION . ":</b> [ <a href='" . XOOPS_URL . '/modules/xfmod/project/admin/group_trove.php?group_id=' . $group->getID() . "'>" . _XF_G_EDIT . '</A> ]</p>'
);
...
while (false !== ($row_memb = $xoopsDB->fetchArray($res_memb))) {
    ...
    $content .= "<FORM ACTION='"
                . XOOPS_URL
                . "/modules/xfmod/project/admin/rmuser.php' METHOD='POST'>"
...
if ($perm->isAdmin()) {
    ...
    $content .= "<HR NoShade SIZE='1'>"
                        ...
                        . XOOPS_URL
                        . '/modules/xfmod/project/admin/userpermedit.php?group_id='
                        . $group->getID()
                        . "' METHOD='POST'>"
    ...
    if ($membermodule && $modpermHandler->checkRight('module', $membermodule->mid(), $xoopsUser->getGroups())) {
        $content .= "COLSPAN='3'>"
                    ...
                    . XOOPS_URL
                    . "/modules/xoopsmembers/?userlookup=yes&iscomm=yes','userlookup','status,scrollbars,height=480,width=640');return false\">User Lookup</a>"
                    . '</TD></TR>'
                    . "<TR><TD COLSPAN='3'";
    } else {
        $content .= "COLSPAN='2'>" . _XF_COMM_HOWTOADDUSER . '</TD></TR>' . "<TR><TD ALIGN='RIGHT'><B>" . _XF_PRJ_USERNAME . ":&nbsp;&nbsp;</B></TD><TD><INPUT TYPE='TEXT' NAME='form_unix_name' SIZE='10' VALUE=''>" . '</TD></TR>' . "<TR><TD COLSPAN='2'";
    }

    $content .= " ALIGN='CENTER'><INPUT TYPE='SUBMIT' NAME='submit' VALUE='" . _XF_PRJ_ADDUSER . "'></TD></TR></FORM>" . '</TABLE>'

                . "<HR NoShade SIZE='1'>" . "<div align='center'>" . "[ <A href='" . XOOPS_URL . '/modules/xfmod/project/admin/userperms.php?group_id=' . $group->getID() . "'>" . _XF_COMM_EDITMEMBERPERMS . '</A> ]' . '</div>';
}
...
$content = "<br><FORM NAME='addprojform' ACTION='" . XOOPS_URL . '/modules/xfmod/community/admin/?group_id=' . $group->getID() . "' METHOD='POST'>" . "<INPUT TYPE='hidden' NAME='func' VALUE='addproj'>" . "<TABLE WIDTH='100%' BORDER='0'>" . "<TR><TD ALIGN='CENTER' COLSPAN='3'>";
$content .= 'To associate a project with this community, enter the project name below and submit</td></tr>'
            ...
            . "</TD><TD ALIGN='LEFT' WIDTH='50%'>&nbsp;&nbsp;<a href=\"#\" onClick=\"window.open('"
            . XOOPS_URL
            . "/modules/xftrove/project_list.php?projlookup=yes&iscomm=yes','projlookup','resizable,status,scrollbars,height=480,width=640');return false\">Project Lookup</a>"
            . '</TD></TR>';
$content .= "<TR><TD COLSPAN='3' ALIGN='CENTER'><INPUT TYPE='SUBMIT' NAME='submit' VALUE='Add Project'></TD></TR></FORM>";
$content .= '<TR><TD>&nbsp;&nbsp;</TD></TR>';
$content .= "<tr><td colspan='3' align='center'>To change the project associations or featured project list:<BR><a href='" . XOOPS_URL . '/modules/xfmod/community/admin/proj_list.php?group_id=' . $group->getID() . "'>" . 'Edit Project List</A>';
$content .= '</TABLE>';
