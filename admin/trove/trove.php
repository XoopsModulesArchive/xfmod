...
if ($xoopsUser->isAdmin($xoopsModule->mid())) {
    ...
    function TroveInsert($shortname, $fullname, $description, $parent)
    {
        ...
        if ($shortname) {
            $res = $xoopsDB->queryF(
                'INSERT INTO ' . $xoopsDB->prefix('xf_trove_cat') . ' (shortname,fullname,description,parent,version,root_parent) VALUES (' . "'$shortname'," . "'$fullname'," . "'$description'," . "'$parent'," . "'" . date('Ymd', time()) . "01'," . "'$newroot')"
            );
    ...
    function TroveEdit($trove_cat_id)
    {
        ...
        echo "<OPTION value='0'" . (0 == $row_cat['parent'] ? ' selected' : '') . ">[ ROOT ]\n";

        while (false !== ($row_parent = $xoopsDB->fetchArray($res_parent))) {
        ...
        echo '</SELECT>'
             . "<input type='hidden' name='trove_cat_id' value='"
             . $GLOBALS['trove_cat_id']
             . "'>"
             . '<p>New category short name (no spaces, unix-like):'
             . "<br><input type='text' name='shortname' value='"
             . $row_cat['shortname']
             . "'>"
             . '<p>New category full name (VARCHAR 80):'
             . "<br><input type='text' name='fullname' value='"
             . $row_cat['fullname']
             . "'>"
             . '<p>New category description (VARCHAR 255):'
             . "<br><input type='text' name='description' size='80' value='"
             . $row_cat['description']
             . "'>"
             . '<br>'
             . "<input type='hidden' name='fct' value='trove'>"
    ...
    function TroveSave($trove_cat_id, $shortname, $fullname, $description, $parent)
    {
        ...
        if ($shortname) {
            $res = $xoopsDB->queryF(
                'UPDATE ' . $xoopsDB->prefix('xf_trove_cat') . ' SET ' . "shortname='$shortname'," . "fullname='$fullname'," . "description='$description'," . "parent='$parent'," . "version='" . date('Ymd', time()) . "01'," . "root_parent='$newroot' " . "WHERE trove_cat_id='$trove_cat_id'"
            );
    ...
    function TroveList_printnode($nodeid, $text)
    {
        ...
        $res_child = $xoopsDB->query(
            'SELECT trove_cat_id,fullname ' . 'FROM ' . $xoopsDB->prefix('xf_trove_cat') . " WHERE parent='$nodeid'"
        );
