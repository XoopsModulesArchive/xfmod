...
if ($xoopsUser->isAdmin($xoopsModule->mid())) {
    ...
    function GroupsMain()
    {
        ...
        for ($i = 0, $iMax = count($abc_array); $i < $iMax; $i++) {
    ...
    function GroupList($search, $substr, $status, $is_public)
    {
      ...
      */
        ...
        $result = $xoopsDB->query(
            'SELECT DISTINCT * ' . 'FROM ' . $xoopsDB->prefix('xf_groups') . ' ' . "WHERE (group_id LIKE '$search' " . "OR unix_group_name LIKE '$search' " . "OR group_name LIKE '$search') " . (string)$crit_sql
        );
        ...
        echo "<p><b>Group search with criteria '<i>" . $search . "</i>' " . $crit_desc . ': ' . $xoopsDB->getRowsNum($result) . ' matches.</b></p>';
        ...
        if ($xoopsDB->getRowsNum($result) < 1) {
            ...
            while (false !== ($row = $xoopsDB->fetchArray($result))) {
                ...
                echo "<tr class='"
                     . (0 != $i++ % 2 ? 'odd' : 'even')
                     . "'>"
                     . "<td><a href='admin.php?fct=groups&op=GroupEdit&group_id="
                     . $row['group_id']
                     . "'>"
                     . $row['group_id']
                     . '</a></td>'
                     . '<td>'
                     . format_name($row['unix_group_name'], $row['status'])
                     . '</td>'
                     . '<td>'
                     . $row['group_name']
                     . '</td>'
                     . '<td>'
                     . date($sys_datefmt, $row['register_time'])
                     . '</td>'
                     . "<td align='center'>"
                     . format_name($row['status'] . $extra_status, $row['status'])
                     . '</td>'
                     . '</tr>';
            }
    ...
    function GroupApprove($action, $list_of_groups, $response_text, $response_title, $add_to_can, $response_id)
    {
        ...
        while (false !== ($row_grp = $xoopsDB->fetchArray($res_grp))) {
            ?>
    ...
    function GroupEdit($group_id, $submit, $resend, $form_public, $form_status, $form_license, $group_type, $form_domain)
    {
        ...
        echo '<H4>'
             ...
             . "[ <A href='"
             . XOOPS_URL
             . '/modules/xfmod/project/admin/?group_id='
             . $group_id
             . "'>Project Admin</A> ]"
             . '<p>'
             . "<FORM action='admin.php' method='POST'>"
             ...
             . 'Public?:'
             . html_build_select_box_from_arrays([0, 1], ['No', 'Yes'], 'form_public', $group->isPublic(), false)
             . '</td>'
             . '</tr>'
             . '<tr>'
             . '<td>'
        ...
        echo '</SELECT>'
             ...
             . '</td>'
             . '<td>'
             . "<INPUT size='40' type='text' name='form_domain' value='"
             . $group->getDomain()
             . "'>"
             . '</td>'
             . '</tr>'
        ...
        if ('other' == $group->getLicense()) {
            echo '<tr>' . '<td>License Other:' . '</td>' . '<td>' . $group->getLicenseOther() . '</td>' . '</tr>';
        }
    ...
    function ListUsers($group_id, $action, $user_id)
    {
        ...
        */
        ...
        $result = $xoopsDB->query(
            'SELECT u.uid AS user_id,u.uname AS user_name ' . 'FROM ' . $xoopsDB->prefix('users') . ' u,' . $xoopsDB->prefix('xf_user_group') . ' ug ' . 'WHERE u.uid=ug.user_id ' . 'AND ug.group_id=' . $group_id . ' ' . 'ORDER BY u.uname'
        );

        echo "<TABLE width='100%' cellspacing='0' cellpadding='0' BORDER='1'>";

        while (false !== ($user = $xoopsDB->fetchArray($result))) {
            echo '<TR>' . '<TD><B>' . $user[user_id] . '</B></TD>' . '<TD>' . $user[user_name] . '</TD>' . "<TD>[ <A HREF='" . XOOPS_URL . '/modules/xfjobs/viewprofile.php?user_id=' . $user[user_id] . "'>View Profile</A> ]</TD>" . '</TR>';
        }
                ...
                if ($result2) {
                    ...
                    while (false !== ($row2 = $xoopsDB->fetchArray($result2))) {
                        ...
                        if ($result3) {
                            ...
                            while (false !== ($row3 = $xoopsDB->fetchArray($result3))) {
