...
?' . '>';
    ...
    $file = fopen(XOOPS_ROOT_PATH . '/modules/xfmod/cache/newsletterconfig.php', 'wb');
...
function topdownloads()
{
    ...
    $sql = 'SELECT g.group_name FROM ' . ' ' . $xoopsDB->prefix('xf_frs_package') . ' AS p, ' . ' ' . $xoopsDB->prefix('xf_frs_release') . ' AS r, ' . ' ' . $xoopsDB->prefix('xf_frs_file') . ' AS f, ' . ' ' . $xoopsDB->prefix('xf_frs_dlstats_file_agg') . ' AS d, ' . ' ' . $xoopsDB->prefix(
        'xf_groups'
    ) . ' AS g ' . ' WHERE p.package_id=r.package_id ' . ' AND r.release_id=f.release_id ' . ' AND f.file_id=d.file_id ' . ' AND p.group_id=g.group_id ' . ' ORDER BY d.downloads DESC LIMIT ' . $limit;

    $result = $xoopsDB->query($sql);

    $message .= '<BR>Top Downloads<BR>';

    $message .= '--------------------<BR>';

    while (false !== ($row = $xoopsDB->fetchArray($result))) {
        $message .= '- ' . $row['group_name'] . '<BR>';
    }
...
function topactive_projects()
{
    ...
    $sql = 'SELECT group_name,grp.group_id,metric.group_id FROM ' . $xoopsDB->prefix('xf_project_weekly_metric') . ' as metric ' . 'left join ' . $xoopsDB->prefix('xf_groups') . ' as grp ' . 'on grp.group_id=metric.group_id ' . 'ORDER BY ranking LIMIT ' . $limit;

    $result = $xoopsDB->query($sql);

    $message .= '<BR>Top 10 Active Projects<BR>';
...
function spotlight_user($user_id)
{
    ...
    $sql = 'SELECT u.name name, u.uname uname, u.user_avatar, u.user_regdate, u.bio, u.user_intrest, ur.rank_title, up.resume '
           ...
           . ' ur, '
           . $xoopsDB->prefix('xf_user_profile')
           . ' up '
           . "WHERE u.uid='"
           . $user_id
           . "' "
    ...
    $message .= '<BR>User Spotlight<BR>';

    $message .= '--------------------<BR>';

    while (false !== ($row = $xoopsDB->fetchArray($result))) {
        $message .= 'User: ' . $row['name'] . '<BR>';

        $message .= 'Name: ' . $row['uname'] . '<BR>';

        //$message = $message."Avatar: ".$row['user_avatar']."<BR>" ;

        $message .= 'Biography: ' . $row['bio'] . '<BR>';

        $message .= 'Interests: ' . $row['user_intrest'] . '<BR>';

        $message .= 'Rank: ' . $row['rank_title'] . '<BR>';

        $message .= 'Resume: ' . $row['resume'] . '<BR>';
    }
...
function spotlight_community($community_id)
{
    ...
    $sql = 'SELECT group_name, homepage, short_description ' . 'FROM ' . $xoopsDB->prefix('xf_groups') . ' ' . "WHERE group_id='" . $community_id . "' " . "AND is_public='1' " . "AND status='A' " . "AND type='2'";

    $result = $xoopsDB->query($sql);

    $message .= '<BR>Community Spotlight<BR>';

    $message .= '--------------------<BR>';

    while (false !== ($row = $xoopsDB->fetchArray($result))) {
        $message .= 'Community Name: ' . $row['group_name'] . '<BR>';

        $message .= 'Home Page: ' . $row['homepage'] . '<BR>';

        $message .= 'Description: ' . $row['short_description'] . '<BR>';
    }
...
function spotlight_project($project_id)
{
    ...
    $sql = 'SELECT group_name, homepage, short_description ' . 'FROM ' . $xoopsDB->prefix('xf_groups') . ' ' . "WHERE group_id='" . $project_id . "' " . "AND is_public='1' " . "AND status='A' " . "AND type='1'";

    $result = $xoopsDB->query($sql);

    $message .= '<BR>Project Spotlight<BR>';

    $message .= '--------------------<BR>';

    while (false !== ($row = $xoopsDB->fetchArray($result))) {
        $message .= 'Project Name: ' . $row['group_name'] . '<BR>';

        $message .= 'Home Page: http://' . $row['homepage'] . '<BR>';
...
function newest_projects()
{
    ...
    $sql = 'SELECT group_name, homepage, short_description ' . 'FROM ' . $xoopsDB->prefix('xf_groups') . ' ' . "WHERE is_public='1' " . "AND status='A' " . "AND type='1' " . 'ORDER BY register_time DESC LIMIT 10';

    $result = $xoopsDB->query($sql);

    $message .= '<BR>Ten Newest Projects<br>';
...
function newest_communities()
{
    ...
    $sql = 'SELECT group_name, homepage, short_description ' . 'FROM ' . $xoopsDB->prefix('xf_groups') . ' ' . "WHERE is_public='1' " . "AND status='A' " . "AND type='2' " . 'ORDER BY register_time DESC LIMIT 10';

    $result = $xoopsDB->query($sql);

    $message .= '<BR>Ten Newest Communities<BR>';
...
function getAllGroupsList($criteria = [], $sort = 'group_id', $order = 'ASC')
{
    global $xoopsDB;
...
function getMessage()
{
    ...
    if ('1' == $xoopsNewsLetterConfig['header_active']) {
        $message .= $xoopsNewsLetterConfig['header_body'] . "\n";
    }

    if ($xoopsNewsLetterConfig['body_active']) {
        $message .= $xoopsNewsLetterConfig['body_body'] . "\n\n";
    }

    if ($xoopsNewsLetterConfig['topdownloads_active']) {
        $message .= topdownloads() . "\n";
    }

    if ($xoopsNewsLetterConfig['topactive_projects_active']) {
        $message .= topactive_projects() . "\n";
    }

    if ($xoopsNewsLetterConfig['spotlight_user_active']) {
        $message .= spotlight_user($xoopsNewsLetterConfig['spotlight_user_id']) . "\n";
    }

    if ($xoopsNewsLetterConfig['spotlight_community_active']) {
        $message .= spotlight_community($xoopsNewsLetterConfig['spotlight_community_id']) . "\n";
    }

    if ($xoopsNewsLetterConfig['spotlight_project_active']) {
        $message .= spotlight_project($xoopsNewsLetterConfig['spotlight_project_id']) . "\n";
    }

    if ($xoopsNewsLetterConfig['newest_projects_active']) {
        $message .= newest_projects() . "\n";
    }

    if ($xoopsNewsLetterConfig['newest_communities_active']) {
        $message .= newest_communities() . "\n";
    }

    if ($xoopsNewsLetterConfig['footer_active']) {
        $message .= $xoopsNewsLetterConfig['footer_body'] . "\n\n";
    }
