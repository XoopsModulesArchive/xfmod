...
function includefile($filename)
{
    ...
    if (file_exists(XOOPS_ROOT_PATH . '/modules/xfmod/language/' . $xoopsConfig['language'] . '/' . $filename)) {
        require XOOPS_ROOT_PATH . '/modules/xfmod/language/' . $xoopsConfig['language'] . '/' . $filename;
    } else {
        require XOOPS_ROOT_PATH . '/modules/xfmod/language/english/' . $filename;
    }
}
...
if ($xoopsUser->isAdmin($xoopsModule->mid())) {
    ...
    if ($submit) {
        ...
        $unix_name = mb_strtolower($unix_name);
            ...
            if (!$res) {
                ...
                */
                ...
                if (!$group->approve($xoopsUser)) {
                    ...
                    echo "<a href='" . XOOPS_URL . '/modules/xfmod/community/?' . $group->getUnixName() . "'>" . XOOPS_URL . '/modules/xfmod/community/?' . $group->getUnixName() . '</a><br>';
    ...
    <hr><h4>Existing Communities</h4><p>
        <?php
        $sql = 'SELECT group_name, unix_group_name, short_description FROM ' . $xoopsDB->prefix('xf_groups') . " WHERE type='2'";
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
        ...
        <BR>
        <INPUT size="40" maxlength="40" type=text name="full_name" value="<?php echo $ts->htmlSpecialChars($full_name); ?>">
...
<?php echo $ts->htmlSpecialChars($description); ?>
</TEXTAREA>
        </font>
        ...
        <P><?php echo _XF_REG_UNIXNAME; ?>:
            <BR>
            <input type=text maxlength="15" SIZE="15" name="unix_name" value="<?php echo $ts->htmlSpecialChars($unix_name); ?>">
