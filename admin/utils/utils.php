...
if ($xoopsUser->isAdmin($xoopsModule->mid())) {
    ...
    function SiteMailings($pattern, $submit, $uid, $uname, $ok)
    {
        ...
        if ($submit && $uid && $uname) {
            if (!$ok) {
                ...
                */
                ...
                echo '<H4>Unsubscribe user: ' . $uname . '</h4>' . '<p>' . 'Are you sure you want to remove this user from all' . 'automated mailings (like forum and file release notifications)?' . '</p>' . '<table><tr><td>' . myTextForm(
                    'admin.php?fct=utils&op=SiteMailings&pattern=' . urlencode($pattern) . '&submit=1&uid=' . $uid . '&uname=' . $uname . '&ok=1',
                    _YES
                ) . '</td><td>' . myTextForm('admin.php?fct=utils&op=SiteMailings&pattern=' . urlencode($pattern), _NO) . '</td></tr></table>';
        ...
        if ($pattern) {
            $res = $xoopsDB->query(
                'SELECT * FROM ' . $xoopsDB->prefix('users') . ' ' . "WHERE level <> 0 AND (uname LIKE '%$pattern%' " . "OR name LIKE '%$pattern%' " . "OR email LIKE '%$pattern%')"
            );

            echo "<table border='0' width='100%'>" . "<tr class='bg2'>" . '<td><b>UID</b></td>' . '<td><b>Username</b></td>' . '<td><b>Email</b></td>' . '</tr>';
            ...
            while (false !== ($row = $xoopsDB->fetchArray($res))) {
                echo "<TR class='"
                     ...
                     . $row['uid']
                     . '</TD>'
                     . "<TD><a href='admin.php?fct=utils&op=SiteMailings&pattern="
                     . urlencode($pattern)
                     . '&submit=1&uid='
                     . $row['uid']
                     . '&uname='
