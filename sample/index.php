...
$GLOBALS['xoopsOption']['template_main'] = 'sample/xfmod_index.html';
...
if ($group_id) {
    ...
    $query = 'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_sample_groups') . ' ' . "WHERE group_id='$group_id' " . 'ORDER BY groupname';
    ...
    if ($xoopsDB->getRowsNum($result) < 1) {
        ...
        while (false !== ($row = $xoopsDB->fetchArray($result))) {
            $query = 'SELECT description, sampleid, title, data, sample_group, createdate, created_by, stateid ' . 'FROM ' . $xoopsDB->prefix('xf_sample_data') . " WHERE sample_group='" . $row['sample_group'] . "'" . " AND (stateid='1'";
            ...
            $content .= '<p><b>' . $ts->htmlSpecialChars($row['groupname']) . "</b>\n<ul>\n";

            if ($xoopsDB->getRowsNum($subresult) > 0) {
                while (false !== ($subrow = $xoopsDB->fetchArray($subresult))) {
                    $private_sample = (1 == $subrow['stateid']) ? false : true;

                    $url = 'index.php?group_id=' . $group_id . '&sampleid=' . $subrow['sampleid'];

                    if ($private_project || $private_sample) {
                        $url .= '&private=1';
                    }

                    $tempUser = $memberHandler->getUser($subrow['created_by']);

                    $name = '' != $tempUser->getVar('name', 'E') ? $tempUser->getVar('name', 'E') : $tempUser->getVar('uname', 'E');

                    $content .= '<li>' . "<b><a href='$url'>" . $ts->htmlSpecialChars($subrow['title']) . '</a></b>' . '<BR>&nbsp;&nbsp;<i>' . _XF_G_DESCRIPTION . ':</i> ' . $ts->htmlSpecialChars($subrow['description']) . '<BR>&nbsp;&nbsp;<i>' . _XF_SC_CREATEDBY . ':</i> ' . $name;

                    if (!mb_strstr($subrow['data'], 'http://')) {
