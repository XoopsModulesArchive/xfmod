...
function sampleman_header($project, $group_id, $pagehead, $style = 'xyz')
{
    ...
    $header .= "<b><a href='" . XOOPS_URL . '/modules/xfmod/sample/index.php?group_id=' . $group_id . "'>" . _XF_SC_VIEWCODE . '</a>' . " | <a href='" . XOOPS_URL . '/modules/xfmod/sample/new.php?group_id=' . $group_id . "'>" . _XF_SC_SUBMITNEWCODE . '</a></b>';
...
function display_groups_option($group_id = false, $checkedval = 'xzxz')
{
    ...
    $query = 'SELECT sample_group, groupname ' . 'FROM ' . $xoopsDB->prefix('xf_sample_groups') . ' ' . "WHERE group_id='$group_id' " . 'ORDER BY groupname';
...
function display_groups($group_id)
{
    ...
    $query = 'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_sample_groups') . ' ' . "WHERE group_id='$group_id'";
    ...
    if ($xoopsDB->getRowsNum($result) < 1) {
        ...
        $content .= "<table border='0' width='100%'>" . "<tr class='bg2'>" . '<td><b>' . _XF_SC_GROUPID . '</b></td>' . '<td><b>' . _XF_SC_GROUPNAME . '</b></td>' . '<td><b>' . _XF_SC_CONTROLS . '</b></td>' . '</tr>';
        ...
        while (false !== ($row = $xoopsDB->fetchArray($result))) {
            $content .= "<tr class='"
                        ...
                        . "</td>\n"
                        . '<td>'
                        . $ts->htmlSpecialChars($row['groupname'])
                        . "</td>\n"
                        . "<td>[ <a href='"
                        . XOOPS_URL
                        . '/modules/xfmod/sample/admin/index.php?mode=groupdelete&sample_group='
                        . $row['sample_group']
                        . '&group_id='
                        . $group_id
                        . "'>"
                        ...
                        . "</A> ] [ <a href='"
                        . XOOPS_URL
                        . '/modules/xfmod/sample/admin/index.php?mode=groupedit&sample_group='
                        . $row['sample_group']
                        . '&group_id='
                        . $group_id
                        . "'>"
...
function display_sample_feedback($group_id, $sampleid, $limit)
{
    ...
    if ($xoopsDB->getRowsNum($res_feedback) > 0) {
        $query = 'SELECT uname,answer,suggestion ' . 'FROM ' . $xoopsDB->prefix('users') . ' u,' . $xoopsDB->prefix('xf_sample_feedback') . ' df ' . 'WHERE u.uid=df.user_id ' . "AND df.sampleid='" . $sampleid . "' " . 'ORDER BY entered DESC';
        ...
        while (false !== ($row = $xoopsDB->fetchArray($res))) {
            $content .= "<tr class='" . ($i % 2 > 0 ? 'bg1' : 'bg3') . "'>" . '<td>' . $row['uname'] . "</td>\n" . '<td>' . $answer_arr[$row['answer']] . '</td>' . '<td>' . $ts->displayTarea($row['suggestion']) . "</td>\n" . "</tr>\n";

            $i++;
        }

        $content .= '<tr>' . "<td colspan='3'>[ <a href='" . XOOPS_URL . '/modules/xfmod/sample/admin/index.php?mode=showfeedback&sampleid=' . $row['sampleid'] . '&group_id=' . $group_id . "'>" . _XF_SC_SHOWALL . '</A> ]</td>' . '</tr></table>';
    } else {
 ...
 *
 * @param The $group_id project group ID
...
function get_group_count($group_id)
{
    ...
    $query = 'SELECT COUNT(*) AS count ' . 'FROM ' . $xoopsDB->prefix('xf_sample_groups') . ' ' . "WHERE group_id='$group_id'";
...
function display_samples($style, $group_id)
{
    ...
    if ($xoopsDB->getRowsNum($result) < 1) {
        $query = 'SELECT name ' . 'FROM ' . $xoopsDB->prefix('xf_sample_states') . ' ' . "WHERE stateid='$style'";
        ...
        $content .= "<table border='0' width='100%'>" . "<tr class='bg2'>" . '<td><b>' . _XF_SC_NAME . '</b></td>' . '<td><b>' . _XF_SC_SAMPLETID . '</b></td>';

        if (2 == $style) {
            $content .= '<td><b>' . _XF_SC_DELETEDBY . '</b></td>';
        }

        $content .= '<td><b>' . _XF_SC_UPDATEDATE . '</b></td>' . '<td><b>' . _XF_SC_CREATEDATE . '</b></td>' . '</tr>';
        ...
        while (false !== ($row = $xoopsDB->fetchArray($result))) {
            ...
            if (2 != $style) {
                $content .= "<a href='" . XOOPS_URL . '/modules/xfmod/sample/admin/index.php?sampleid=' . $row['sampleid'] . '&mode=sampleedit&group_id=' . $group_id . "'>";
            }

            $content .= $ts->htmlSpecialChars($row['title']);
