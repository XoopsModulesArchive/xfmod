...
$GLOBALS['xoopsOption']['template_main'] = 'news/admin/xfmod_index.html';
    ...
    */
    ...
    if ($post_changes) {
        if ($approve) {
            ...
            */
            ...
            $sql = 'UPDATE ' . $xoopsDB->prefix('xf_news_bytes') . ' SET ' . "is_approved='$status'," . "summary='" . $ts->addSlashes($summary) . "'," . "details='" . $ts->addSlashes($details) . "' " . "WHERE id='$id' " . "AND group_id='$group_id'";
    ...
    if ($approve) {
        ...
        */
		...
		<INPUT TYPE="TEXT" NAME="summary" VALUE="' . $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'summary')) . '" SIZE="30" MAXLENGTH="60"><BR>
		<B>' . _XF_NWS_DETAILS . ':</B><BR>
		<TEXTAREA NAME="details" ROWS="5" COLS="50" WRAP="SOFT">' . $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'details')) . '</TEXTAREA><P>
        ...
        */
        ...
        if ($rows < 1) {
            ...
            for ($i = 0; $i < $rows; $i++) {
                $content .= '
				<A HREF="' . XOOPS_URL . '/modules/xfmod/news/admin/?approve=1&id=' . unofficial_getDBResult($result, $i, 'id') . '&group_id=' . unofficial_getDBResult($result, $i, 'group_id') . '">' . $ts->htmlSpecialChars(unofficial_getDBResult($result, $i, 'summary')) . '</A><BR>';
            }
        }
    }
    ...
    */
        ...
        if ($approve) {
            if (1 == $status) {
                ...
                */
                ...
                $sql = 'UPDATE ' . $xoopsDB->prefix('xf_news_bytes') . ' SET ' . "is_approved='1'," . "date='" . time() . "'," . "summary='" . $ts->addSlashes($summary) . "'," . "details='" . $ts->addSlashes($details) . "' " . "WHERE id='$id'";
            ...
            */
            ...
            $sql = 'UPDATE ' . $xoopsDB->prefix('xf_news_bytes') . ' SET ' . "is_approved='2' " . "WHERE id IN ('" . implode("','", $news_id) . "')";
    ...
    if ($approve) {
        ...
        */
        ...
        $sql = 'SELECT g.unix_group_name,g.group_name,u.uname,nb.* ' . 'FROM ' . $xoopsDB->prefix('xf_news_bytes') . ' nb,' . $xoopsDB->prefix('xf_groups') . ' g,' . $xoopsDB->prefix('users') . ' u ' . "WHERE id='$id' " . 'AND nb.submitted_by=uid ' . 'AND nb.group_id=g.group_id';
		...
		<INPUT TYPE="TEXT" NAME="summary" VALUE="' . $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'summary')) . '" SIZE="30" MAXLENGTH="60"><BR>
		<B>' . _XF_NWS_DETAILS . ':</B><BR>
		<TEXTAREA NAME="details" ROWS="5" COLS="50" WRAP="SOFT">' . $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'details')) . '</TEXTAREA><BR>
		<INPUT TYPE="SUBMIT" NAME="SUBMIT" VALUE="' . _XF_G_SUBMIT . '">
		</FORM>'
        );
    } else {
        ...
        */
        ...
        $sql_rejected = 'SELECT g.group_id,id,date,summary,group_name,unix_group_name ' . 'FROM ' . $xoopsDB->prefix('xf_news_bytes') . ' nb,' . $xoopsDB->prefix('xf_groups') . ' g ' . 'WHERE is_approved=2 ' . 'AND nb.group_id=g.group_id ' . "AND date > '$old_date' " . 'ORDER BY date';

        $sql_approved = 'SELECT g.group_id,id,date,summary,group_name,unix_group_name ' . 'FROM ' . $xoopsDB->prefix('xf_news_bytes') . ' nb,' . $xoopsDB->prefix('xf_groups') . ' g ' . 'WHERE is_approved=1 ' . 'AND nb.group_id=g.group_id ' . "AND date > '$old_date' " . 'ORDER BY date';
