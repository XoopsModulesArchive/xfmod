...
$GLOBALS['xoopsOption']['template_main'] = 'news/xfmod_index.html';
...
if ($group_id && ($group_id != $xoopsForge['sysnews'])) {
    $sql = 'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_news_bytes') . ' ' . "WHERE group_id='$group_id' " . "AND is_approved<>'4' " . 'ORDER BY date DESC';
} else {
    $sql = 'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_news_bytes') . ' ' . "WHERE is_approved='1' " . 'ORDER BY date DESC';
}
...
if ($rows < 1) {
    ...
    for ($j = 0; $j < $rows; $j++) {
        $content .= '
		<A HREF="' . XOOPS_URL . '/modules/xfmod/forum/forum.php?forum_id=' . unofficial_getDBResult($result, $j, 'forum_id') . '">' . '<img src="' . XOOPS_URL . '/modules/xfmod/images/ic/cfolder15.png" width="15" height="13" BORDER="0" alt="news thread"> &nbsp;' . $ts->htmlSpecialChars(
            unofficial_getDBResult($result, $j, 'summary')
        ) . '</A> <BR>';
    }
