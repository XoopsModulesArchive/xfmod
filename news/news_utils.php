...
function news_show_latest($group_id = '', $limit = 10, $show_summaries = true, $allow_submit = true, $flat = false, $tail_headlines = 0)
{
    ...
    */
    ...
    if (!$result || $rows < 1) {
        ...
        for ($i = 0; $i < $rows; $i++) {
            if ($show_summaries && $limit) {
                ...
                if (mb_strlen($summ_txt) > 200) {
                    $summ_txt = mb_substr($summ_txt, 0, 200) . '...';
                }

                //show the project name

                if (2 == unofficial_getDBResult($result, $i, 'type')) {
                    $group_type = '/foundry';
                } else {
                    $group_type = '/project';
                }

                $proj_name = "&nbsp;&nbsp; <A HREF='" . XOOPS_URL . '/xf' . $group_type . '/?group_id=' . unofficial_getDBResult($result, $i, 'group_id') . "'>" . $ts->htmlSpecialChars(unofficial_getDBResult($result, $i, 'group_name')) . '</A>';
            } else {
                $proj_name = '';
            ...
            if (!$limit) {
                $return .= "<li><A HREF='" . XOOPS_URL . '/modules/xfmod/forum/forum.php?forum_id=' . unofficial_getDBResult($result, $i, 'forum_id') . "'><B>" . $ts->htmlSpecialChars(unofficial_getDBResult($result, $i, 'summary')) . '</B></A>';
                ...
                $return .= "<table align='center' border='0' cellspacing='0' cellpadding='0' width='96%'>"
                           . '<tr>'
                           . "<td width='2%' valign='bottom'><img src='"
                           . XOOPS_URL
                           . "/modules/xfmod/images/h_arrow.gif' width='14' height='26' border='0' alt=''></td>"
                           . "<td align='left' valign='bottom' width='98%'><A HREF='"
                           . XOOPS_URL
                           . '/modules/xfmod/forum/forum.php?forum_id='
                           . unofficial_getDBResult($result, $i, 'forum_id')
                           . "'><span class='head3'>"
                           . unofficial_getDBResult($result, $i, 'summary')
                           . '</span></A></td></tr><tr>'
                           . "<td colspan='2' background='"
                           . XOOPS_URL
                           . "/modules/xfmod/images/dotlinebg_horiz.gif'><img src='"
                           . XOOPS_URL
                           . "/modules/xfmod/images/spacer.gif' width='1' height='1' border='0' alt=''></td>"
                ...
                */
                ...
                $sql = 'SELECT COUNT(f.msg_id) AS count ' . 'FROM ' . $xoopsDB->prefix('xf_forum') . ' f ' . 'WHERE f.group_forum_id=' . unofficial_getDBResult($result, $i, 'forum_id');
    ...
    $return .= "<div align='center'><BR>" . "[ <a href='" . XOOPS_URL . '/modules/xfmod' . $archive_url . "'>" . _XF_NWS_NEWSARCHIVE . '</a> ]</div>';
    ...
    if (!$result || $rows < 1) {
        ...
        for ($i = 0; $i < $rows; $i++) {
            if ($show_summaries) {
                ...
                $proj_name = ' &nbsp; - &nbsp; <A HREF="/projects/' . mb_strtolower(db_result($result, $i, 'unix_group_name')) . '/">' . db_result($result, $i, 'group_name') . '</A>';
            } else {
                $proj_name = '';
