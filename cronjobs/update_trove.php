...
$sql = 'INSERT INTO '
       . $dbprefix
       . '_xf_trove_agg (trove_cat_id,group_id,group_name,unix_group_name,status,register_time,short_description,percentile,ranking) '
       ...
       . 'LEFT JOIN '
       . $dbprefix
       . '_xf_project_weekly_metric pwm USING (group_id)'
       . ','
       . $dbprefix
       . '_xf_trove_group_link tgl '
...
$sql = 'SELECT tc.trove_cat_id,tc.parent,count(g.group_id) AS count '
        ...
        . 'LEFT JOIN '
        . $dbprefix
        . '_xf_trove_group_link tgl ON tc.trove_cat_id=tgl.trove_cat_id '
        . 'LEFT JOIN '
        . $dbprefix
        . '_xf_groups g ON g.group_id=tgl.group_id '
...
function get_trove_sub_projects($cat_id)
{
    ...
    $sum_totals[(string)$cat_id] = $count;
