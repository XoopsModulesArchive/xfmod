...
$rel = db_query(
    'SELECT agl.group_artifact_id ' . 'FROM ' . $dbprefix . '_xf_artifact_group_list agl ' //               ."LEFT JOIN ".$dbprefix."_xf_artifact a USING (group_artifact_id) "
    . 'GROUP BY agl.group_artifact_id;'
);

while (false !== ($rel_arr = db_fetch_array($rel))) {
