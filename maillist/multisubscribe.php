    ...
    if ($maillistcount >= 1) {
        ...
        for ($x = 1; $x <= $maillistcount; $x++) {
            ...
            if ('y' == $subscribeme) {
                ...
                } elseif (!maillist_subscribe(0, 0, $_SERVER['SERVER_NAME'], $list, 0, $email, $pw)) {
                    $feedback = _XF_ML_SUBFAILED;
                }
            }
        }

        echo '<h5>' . _XF_ML_NOWSUBDTO . ' ' . $lists . '</h5>' . '<br><br>' . '<a href="' . XOOPS_URL . "/modules/xfmod/$type/?" . $project->getUnixName() . '">' . _LOCAL_XF_ML_RETURNTOPRJPAGE . '</a><BR>';
