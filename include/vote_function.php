...
function vote_get_rating($id, $flag)
{
    ...
    if (!$result || (db_numrows($result) < 1) || (0 == db_result($result, 0, 0))) {
        return '0';
    }
...
function vote_show_release_radios($vote_on_id, $flag)
{
    ...
    */
    ...
    <FONT SIZE="-2">
        ...
        <FORM ACTION="/survey/rating_resp.php" METHOD="POST">
            ...
            <CENTER>
                ...
                <?php echo html_image('images/rateit.png', '100', '9', []); ?>
    ...
    echo html_image('images/ic/caret.png', '9', '6', []); ?>
...
function show_survey($group_id, $survey_id)
{
    ...
    if ($xoopsDB->getRowsNum($result) > 0) {
        ...
        $quest_array = preg_preg_split("/[\s,]+/", $questions);
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if ('4' == $question_type) {
                ...
                if (($question_type != $last_question_type) && (('1' == $question_type) || ('3' == $question_type))) {
                    $return .= '&nbsp;<BR>';
                }
            ...
            if ('1' == $question_type) {
                ...
                $return .= '&nbsp; ' . $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'question'));
            } elseif ('2' == $question_type) {
                //			  	This is a text-area question.

                $return .= $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'question')) . '<BR>';

                $return .= "<textarea name='_" . $quest_array[$i] . "' rows='5' cols='60' wrap='soft'></textarea>";
            } elseif ('3' == $question_type) {
                ...
                $return .= '&nbsp; ' . $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'question'));
            } elseif ('4' == $question_type) {
                //				  This is a comment only.

                $return .= '&nbsp;<BR><B>' . $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'question')) . '</B>';

                $return .= "<INPUT TYPE='HIDDEN' NAME='_" . $quest_array[$i] . "' VALUE='-666'>";
            } elseif ('5' == $question_type) {
...
function show_survey_small($group_id, $survey_id)
{
    ...
    $return .= "<FORM ACTION='" . XOOPS_URL . "/modules/xfmod/survey/survey_resp.php' METHOD='POST'>" . "<INPUT TYPE='HIDDEN' NAME='group_id' VALUE='" . $group_id . "'>" . "<INPUT TYPE='HIDDEN' NAME='survey_id' VALUE='" . $survey_id . "'>";
    ...
    if ($xoopsDB->getRowsNum($result) > 0) {
        ...
        $return .= "<TR><TD ALIGN='MIDDLE' COLSPAN='2'>"
                   ...
                   . "'>"
                   . '<BR>'
                   . "<A ONCLICK='javascript:open(\""
                   . XOOPS_URL
                   . "/modules/xfmod/survey/privacy.php\",\"Privacy\",\"height=600,width=700,scrollbars=yes,resizable=yes\"); return false;' HREF='#'>"
                   . _XF_SURVEYPRIVACY
                   . '</A>'
                   . '</TD></TR>'
                   . '</FORM>';
...
function vote_show_user_rate_box($user_id, $by_id = 0)
{
    ...
    for ($i = 1, $iMax = count($USER_RATING_QUESTIONS); $i <= $iMax; $i++) {
