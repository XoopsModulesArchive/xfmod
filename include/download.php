...
if ($dl && $dl > 0) {
    ...
    if (mb_strstr($url, '://')) {
        ...
        if ($result && $xoopsDB->getRowsNum($result) > 0) {
            if (!$xoopsForge['ftp_server'] || 'localhost' == $xoopsForge['ftp_server'] || '127.0.0.1' == $xoopsForge['ftp_server']) {
            ...
            if ($_GET['private']) {
                if (!$xoopsForge['ftp_internal_server']) {
                    $xoopsForge['ftp_internal_server'] = $xoopsForge['ftp_server'];
                }

                $url = 'ftp://' . $xoopsForge['ftp_user'] . ':' . $xoopsForge['ftp_password'] . '@' . $xoopsForge['ftp_internal_server'] . '/' . $xoopsForge['ftp_path'] . '/' . $project->getUnixName()                        //project short name
                           . '/' . unofficial_getDBResult($result, 0, 'pname')        //package name
                           . '/' . unofficial_getDBResult($result, 0, 'rname')        //release name
                           . '/' . unofficial_getDBResult($result, 0, 'file_url');    //file name
                    header('Content-Type: application/x-download');
                ...
                $url = 'http://' . $xoopsForge['ftp_server'] . '/' . $xoopsForge['ftp_prefix'] . '/' . $project->getUnixName()                        //project short name
    ...
    if ($result && $xoopsDB->getRowsNum($result) > 0) {
        ...
        if (mb_strstr($url, '://')) {
            ...
            if (!$xoopsForge['ftp_server'] || 'localhost' == $xoopsForge['ftp_server'] || '127.0.0.1' == $xoopsForge['ftp_server']) {
                $url = '/' . $xoopsForge['ftp_path'] . '/' . $project->getUnixName() . '/sample' . '/' . unofficial_getDBResult($result, 0, 'data');
            ...
            if ($_GET['private']) {
                $url = 'ftp://' . $xoopsForge['ftp_user'] . ':' . $xoopsForge['ftp_password'] . '@' . $xoopsForge['ftp_internal_server'] . '/' . $xoopsForge['ftp_path'] . '/' . $project->getUnixName() . '/sample' . '/' . unofficial_getDBResult($result, 0, 'data');
                ...
                $url = 'http://' . $xoopsForge['ftp_server'] . '/' . $xoopsForge['ftp_prefix'] . '/' . $project->getUnixName() . '/sample' . '/' . rawurlencode(unofficial_getDBResult($result, 0, 'data'));
    ...
    if ($result && $xoopsDB->getRowsNum($result) > 0) {
        ...
        if (mb_strstr($url, '://')) {
            ...
            if (!$xoopsForge['ftp_server'] || 'localhost' == $xoopsForge['ftp_server'] || '127.0.0.1' == $xoopsForge['ftp_server']) {
                $url = '/' . $xoopsForge['ftp_path'] . '/' . $project->getUnixName() . '/docs' . '/' . unofficial_getDBResult($result, 0, 'data');
            ...
            if ($_GET['private']) {
                $url = 'ftp://' . $xoopsForge['ftp_user'] . ':' . $xoopsForge['ftp_password'] . '@' . $xoopsForge['ftp_internal_server'] . '/' . $xoopsForge['ftp_path'] . '/' . $project->getUnixName() . '/docs' . '/' . unofficial_getDBResult($result, 0, 'data');
                ...
                $url = 'http://' . $xoopsForge['ftp_server'] . '/' . $xoopsForge['ftp_prefix'] . '/' . $project->getUnixName() . '/docs' . '/' . rawurlencode(unofficial_getDBResult($result, 0, 'data'));
      ...
      if ( "text/html" != $mime_type &&
          ...
          if ($answer)
        {
          ...
          $res = $xoopsDB->queryF("INSERT INTO ".$xoopsDB->prefix("xf_sample_feedback")." "
                      ...
                      ."('$sampleid','$user_id','$answer','".$ts->addSlashes($suggestion)."',".time().")");
          ...
          while ( !feof ($samplefile) )
        {
          ...
          echo $ts->displayTarea($data, 1, 1, 1);
          echo "</td><td width='1'>&nbsp;"
