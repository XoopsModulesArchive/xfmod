...
function start_graph($vals, $names)
{
    ...
    if (($vals['vlabel']) || ($vals['hlabel'])) {
        if ((0 == $vals['type']) || (2 == $vals['type'])) {// horizontal chart
            $rowspan = count($names) + 1;

            $colspan = 3;
        } elseif (1 == $vals['type'] || (3 == $vals['type'])) { // vertical chart
            $rowspan = 3;
    ...
    for ($i = 0, $iMax = count($values); $i < $iMax; $i++) {
        ...
        <TR>
            ...
            if (!$vals['background']) {
                ...
                echo $vals['namefcolor']; ?>" STYLE="<?php
...
function double_vertical_graph($names, $values, $bars, $vals, $dvalues, $dbars)
{
    ...
    for ($i = 0, $iMax = count($values); $i < $iMax; $i++) {
        ...
        <IMG SRC="<?php echo $bars[$i] ?>" WIDTH=10 HEIGHT="<?php if (0 != $values[$i]) {
            echo $values[$i] * $vals['scale'];
        } else {
            echo '1';
        } ?>" alt="">
        ...
        <IMG SRC="<?php echo $dbars[$i] ?>" WIDTH=10 HEIGHT="<?php if (0 != $dvalues[$i]) {
...
function horizontal_multisection_graph($names, $multi_rows, $colors, $vals, $additive = false)
{
    ...
    for ($i = 0, $iMax = count($names); $i < $iMax; $i++) {
        ...
        for ($j = 0; $j < $subbars_num; $j++) {
            ...
            if ($width <= 0 && ($j != $subbars_num - 1 || shown)) {
                continue;
            }
        ...
        if (!$vals['noshowvals']) {
            ...
            for ($j = 0, $jMax = count($multi_rows); $j < $jMax; $j++) {
                if ($j) {
                    print '/';
                }
