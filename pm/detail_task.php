 ...
 */
$GLOBALS['xoopsOption']['template_main'] = 'pm/xfmod_detail_task.html';
...
$content .= "
  ...
  <TR>
		...
		<BR>';
$content .= $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'summary'));
$content .= "
		</TD>
	</TR>
	...
	<TR>
		...
		<P>';
$content .= $ts->displayTarea(unofficial_getDBResult($result, 0, 'details'));
$content .= "
		</TD>
	</TR>
...
$result2 = $xoopsDB->query(
    'SELECT u.uname AS User_Name ' . 'FROM ' . $xoopsDB->prefix('users') . ' u,' . $xoopsDB->prefix('xf_project_assigned_to') . ' pat ' . 'WHERE u.uid=pat.assigned_to_id ' . "AND project_task_id='$project_task_id'"
);
...
*/
$result2 = $xoopsDB->query(
    'SELECT pt.summary ' . 'FROM ' . $xoopsDB->prefix('xf_project_dependencies') . ' pd,' . $xoopsDB->prefix('xf_project_task') . ' pt ' . 'WHERE is_dependent_on_task_id=pt.project_task_id ' . "AND pd.project_task_id='$project_task_id'"
);
