...
$GLOBALS['xoopsOption']['template_main'] = 'pm/admin/xfmod_index.html';
    ...
    */
    ...
    if ($projects) {
        ...
        */
        ...
        $sql = 'INSERT INTO ' . $xoopsDB->prefix('xf_project_group_list') . ' (group_id,project_name,is_public,description) ' . "VALUES ('$group_id','" . $ts->addSlashes($project_name) . "','$is_public','" . $ts->addSlashes($description) . "')";
...
if ($projects) {
  ...
  <INPUT TYPE='SUBMIT' NAME='SUBMIT' VALUE='" . _XF_G_SUBMIT . "'>
    ...
    */
    ...
    $sql = 'SELECT project_name,group_project_id,is_public,description ' . 'FROM ' . $xoopsDB->prefix('xf_project_group_list') . ' ' . "WHERE group_id='$group_id'";
    ...
    <P>' . _XF_PM_MAKEPRIVATEINFO . '<P>';
        ...
        $content .= "<table border='0' width='100%'>" . "<tr class='bg2'>" . '<td><b>' . _XF_PM_STATUS . '</b></td>' . '<td><b>' . _XF_PM_NAME . '</b></td>' . '<td><b>' . _XF_G_UPDATE . '</b></td>' . '</tr>';
        ...
        for ($i = 0; $i < $rows; $i++) {
            ...
            $content .= '<TR class="' . ($j % 2 > 0 ? 'bg1' : 'bg3') . '"><TD>
              <FONT SIZE="-1">
              <B>' . _XF_G_ISPUBLIC . '</B><BR>
              <INPUT TYPE="RADIO" NAME="is_public" VALUE="1"' . (('1' == unofficial_getDBResult($result, $i, 'is_public')) ? ' CHECKED' : '') . '> ' . _YES . '<BR>
              <INPUT TYPE="RADIO" NAME="is_public" VALUE="0"' . (('0' == unofficial_getDBResult($result, $i, 'is_public')) ? ' CHECKED' : '') . '> ' . _NO . '<BR>
              <INPUT TYPE="RADIO" NAME="is_public" VALUE="9"' . (('9' == unofficial_getDBResult($result, $i, 'is_public')) ? ' CHECKED' : '') . '> ' . _XF_G_DELETED . '<BR>
            </TD><TD>
              <INPUT TYPE="TEXT" NAME="project_name" VALUE="' . $ts->htmlSpecialChars(unofficial_getDBResult($result, $i, 'project_name')) . '">
            </TD><TD>
              <FONT SIZE="-1">
            ...
            <TR class="' . ($j % 2 > 0 ? 'bg1' : 'bg3') . '"><TD COLSPAN="3">
              <B>' . _XF_G_DESCRIPTION . ':</B><BR>
              <INPUT TYPE="TEXT" NAME="description" VALUE="' . $ts->htmlSpecialChars(unofficial_getDBResult($result, $i, 'description')) . '" SIZE="40" MAXLENGTH="80"><BR>
            </TD></TR>
            </FORM>';
        }
