...
if ($group_id && $group_project_id) {
    ...
    $result = $xoopsDB->query(
        'SELECT * FROM ' . $xoopsDB->prefix('xf_project_group_list') . ' ' . "WHERE group_project_id='$group_project_id' " . "AND group_id='$group_id' " . "AND is_public IN ($public_flag)"
    );
    ...
    */
    ...
    switch ($func) {
        ...
        case 'postaddtask':
            {
                if ($xoopsUser && ($perm->isPMAdmin() || $perm->isSuperUser())) {
                    if (pm_data_create_task(
                        ...
                        redirect_header(
                            $_SERVER['PHP_SELF'] . "?group_id=$group_id" . "&group_project_id=$group_project_id" . "&func=browse&set=open&feedback=$feedback",
                            0,
                            ''
                        );
                        ...
                        //require __DIR__ . '/browse_task.php';
                    }  
        ...
        case 'detailtask':
            {
                if ($xoopsUser && ($perm->isPMAdmin() || $perm->isSuperUser())) {
                    require __DIR__ . '/mod_task.php';
                } else {
                    require __DIR__ . '/detail_task.php';
                }
            }
            break;
        case 'browse':
