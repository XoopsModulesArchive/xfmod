...
function pm_header($group, $perm, $title, $group_project_id)
{
...
function pm_multiple_task_depend_box($name = 'dependent_on[]', $group_project_id = false, $project_task_id = false)
{
    ...
    return html_build_multiple_select_box($result, $name, []);
}
...
function pm_show_month_box($name, $select_month = 0)
{
    ...
    for ($i = 1, $iMax = count($monthlist); $i <= $iMax; $i++) {
        if ($i == $select_month) {
            $content .= '
...
function pm_show_tasklist($result, $offset, $set = 'open')
{
    ...
    */
    ...
    for ($i = 0; $i < $rows; $i++) {
        $content .= '<TR BGCOLOR="'
                    ...
                    . '</TD>'
                    . '<TD>'
                    . date('Y-m-d', unofficial_getDBResult($result, $i, 'start_date'))
                    . '</TD>'
                    . '<TD>'
                    . (($now > unofficial_getDBResult($result, $i, 'end_date')) ? '<B>* ' : '&nbsp; ')
                    . date('Y-m-d', unofficial_getDBResult($result, $i, 'end_date'))
                    . '</TD>'
                    . '<TD>';
    ...
    */
    ...
    if ($offset > 0) {
        $content .= '<A HREF="' . $_SERVER['PHP_SELF'] . '?func=browse&group_project_id=' . $group_project_id . '&set=' . $set . '&group_id=' . $group_id . '&offset=' . ($offset - 50) . '"><B><-- ' . _XF_G_PREVIOUS . ' 50</B></A>';
    } else {
        $content .= '&nbsp;';
    }
    ...
    if (50 == $rows) {
        $content .= '<A HREF="' . $_SERVER['PHP_SELF'] . '?func=browse&group_project_id=' . $group_project_id . '&set=' . $set . '&group_id=' . $group_id . '&offset=' . ($offset + 50) . '"><B>' . _XF_G_NEXT . ' 50 --></B></A>';
    } else {
        $content .= '&nbsp;';
    }
...
function pm_show_dependent_tasks($project_task_id, $group_id, $group_project_id)
{
    ...
    $sql = 'SELECT pt.project_task_id,pt.summary ' . 'FROM ' . $xoopsDB->prefix('xf_project_task') . ' pt,' . $xoopsDB->prefix('xf_project_dependencies') . ' pd ' . 'WHERE pt.project_task_id=pd.project_task_id ' . "AND pd.is_dependent_on_task_id='$project_task_id'";
    ...
    if ($rows > 0) {
        ...
        $content .= "<table border='0' width='100%'>" . "<tr class='bg2'>" . '<td><b>' . _XF_PM_TASKID . '</b></td>' . '<td><b>' . _XF_G_SUMMARY . '</b></td>' . '</tr>';

        for ($i = 0; $i < $rows; $i++) {
            $content .= '<TR class="' . ($j % 2 > 0 ? 'bg1' : 'bg3') . '">
            <TD><A HREF="' . XOOPS_URL . '/modules/xfmod/pm/task.php?func=detailtask&project_task_id=' . unofficial_getDBResult($result, $i, 'project_task_id') . '&group_id=' . $group_id . '&group_project_id=' . $group_project_id . '">' . unofficial_getDBResult($result, $i, 'project_task_id') . '</TD>
            <TD>' . $ts->htmlSpecialChars(unofficial_getDBResult($result, $i, 'summary')) . '</TD></TR>';
        }
...
function pm_show_task_details($project_task_id)
{
    ...
    if ($rows > 0) {
        ...
        $content .= "<table border='0' width='100%'>" . "<tr class='bg2'>" . '<td><b>' . _XF_G_COMMENT . '</b></td>' . '<td><b>' . _XF_G_DATE . '</b></td>' . '<td><b>' . _XF_G_BY . '</b></td>' . '</tr>';

        for ($i = 0; $i < $rows; $i++) {
            $content .= '<TR class="' . ($i % 2 > 0 ? 'bg1' : 'bg3') . '">
            <TD>' . $ts->htmlSpecialChars(unofficial_getDBResult($result, $i, 'old_value')) . '</TD>
...
function pm_show_task_history($project_task_id)
{
    ...
    */
    ...
    if ($rows > 0) {
        ...
        $content .= "<table border='0' width='100%'>" . "<tr class='bg2'>" . '<td><b>' . _XF_G_FIELD . '</b></td>' . '<td><b>' . _XF_G_OLDVALUE . '</b></td>' . '<td><b>' . _XF_G_DATE . '</b></td>' . '<td><b>' . _XF_G_BY . '</b></td>' . '</tr>';
