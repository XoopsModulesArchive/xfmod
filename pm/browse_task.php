 ...
 */
$GLOBALS['xoopsOption']['template_main'] = 'pm/xfmod_browse_task.html';
...
if ($order) {
    ...
    $order_by = " ORDER BY pt.$order" . (('priority' == $order) ? ' DESC ' : ' ');
} else {
    $order_by = '';
}
...
if ($_status && (100 != $_status)) {
    ...
    $status_str = "AND pt.status_id IN ($_status" . ((1 == $_status) ? ',100' : '') . ')';
} else {
...
$sql = 'SELECT pt.priority,pt.group_project_id,pt.project_task_id,' . 'pt.start_date,pt.end_date,pt.percent_complete,pt.summary ' . 'FROM ' . $xoopsDB->prefix('xf_project_task') . " pt $assigned_table " . "WHERE pt.group_project_id='$group_project_id' " . " $assigned_str $status_str " . $order_by;
...
*/
$content .= '<TABLE WIDTH="10%" BORDER="0"><FORM ACTION="'
            . $_SERVER['PHP_SELF']
            . '" METHOD="GET">
      ...
      <TR><TD COLSPAN="4" nowrap><b>'
            . _XF_PM_BROWSTASKBYUSERSTATUS
            . ':</b></TD></TR>
      <TR><TD>'
            . pm_show_subprojects_box('group_project_id', $group_id, $group_project_id)
            . '</TD>'
            . '<TD><FONT SIZE="-1">'
            . $tech_box
            . '</TD><TD><FONT SIZE="-1">'
            . pm_status_box('_status', $_status, _XF_G_ANY)
            . '</TD>'
            . '<TD><FONT SIZE="-1"><INPUT TYPE="SUBMIT" NAME="SUBMIT" VALUE="'
            . _XF_G_BROWSE
            . '"></TD></TR></FORM></TABLE>';
