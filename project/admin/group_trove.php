...
$GLOBALS['xoopsOption']['template_main'] = 'project/admin/xfmod_group_trove.html';
...
if ($submit && $root1) {
    ...
    while (list($rootnode, $value) = each($root1)) {
        ...
        $xoopsDB->queryF(
            'DELETE FROM ' . $xoopsDB->prefix('xf_trove_group_link') . ' ' . "WHERE group_id='$group_id' " . "AND trove_cat_root='$rootnode'"
        );
...
while (list($catroot, $fullname) = each($CATROOTS)) {
    ...
    $res_grpcat = $xoopsDB->queryF(
        'SELECT trove_cat_id ' . 'FROM ' . $xoopsDB->prefix('xf_trove_group_link') . ' ' . "WHERE group_id='$group_id' " . "AND trove_cat_root='$catroot'"
    );
