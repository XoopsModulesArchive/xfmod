...
$GLOBALS['xoopsOption']['template_main'] = 'project/admin/xfmod_editpackages.html';
...
if (isset($submit) && $submit) {
    ...
    if ('add_package' == $func && $package_name && $perm->isAdmin()) {
        $frs->frsAddPackage($package_name, $group->getUnixName());

        $feedback .= ' ' . $frs->getErrorMessage() . ' ';
    } elseif ('update_package' == $func && $package_id && $package_name && $status_id && $perm->isAdmin()) {
        $frs->frsChangePackage($group, $package_id, $package_name, $status_id);

        $feedback .= ' ' . $frs->getErrorMessage() . ' ';
    } elseif ('delete_package' == $func && $package_id && $perm->isAdmin()) {
        $frs->frsDeletePackage($package_id, $group->getUnixName());

        if ($frs->isError()) {
            $feedback .= ' ' . $frs->getErrorMessage() . ' ';
        }
    } elseif ('delete_release' == $func && $release_id && $perm->isReleaseTechnician()) {
...
*/
$res = $xoopsDB->query(
    'SELECT status_id,package_id,name AS package_name ' . 'FROM ' . $xoopsDB->prefix('xf_frs_package') . ' ' . "WHERE group_id='$group_id'"
);
...
for ($i = 0; $i < $rows; $i++) {
    ...
    if (2 != $status_id || $perm->isReleaseAdmin()) {
        ...
        if ($perm->isAdmin()) {
            ...
            $content .= '<TD><b>' . $package_name . '</b></TD>' . '<TD><b>' . frs_get_status_name($status_id) . '</b></TD>' . '<TD>&nbsp;</TD>' . '<TD>&nbsp;</TD>';
        }
