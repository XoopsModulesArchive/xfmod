...
$GLOBALS['xoopsOption']['template_main'] = 'project/admin/xfmod_index.html';
...
if (isset($feedback) && mb_strlen($feedback) > 0) {
...
if ($perm->isAdmin() && $func) {
    ...
    if ('adduser' == $func) {
        ...
        $results = $xoopsDB->queryF(
            'DELETE FROM ' . $xoopsDB->prefix('xf_trove_group_link') . " WHERE trove_cat_id=$comm_id" . " AND group_id=$group_id"
        );

        if (!$results) {
            $xoopsForgeErrorHandler->addError(
                "Failed to delete association to community $comm_name: " . $xoopsDB->error()
            );
        } else {
    ...
    if ($doLDAPTrans) {
        ...
        $sql = 'SELECT email from ' . $xoopsDB->prefix('xf_cvs_commit_notify') . " WHERE group_id=$group_id";
        ...
        if (!$rc) {
            $xoopsForgeErrorHandler->addError(
                'Failed to add Project notification transaction. ' . 'Error: ' . $lldap->lastError()
            );
        }
    }
}
...
$content = '<p>'
           . _XF_G_DESCRIPTION
           . ': '
           . $ts->displayTarea($group->getDescription())
           . '<br></p>'
           . _XF_G_HOMEPAGE
           . ': '
           . $group->getHomepage()
           ...
           . '<HR NOSHADE>'
           . '<p><b>'
           . _XF_PRJ_TROVECATEGORIZATION
           . ":</b> [ <a href='"
           . XOOPS_URL
           . '/modules/xfmod/project/admin/group_trove.php?group_id='
           . $group->getID()
           . "'>"
           . _XF_G_EDIT
           . '</A> ]</p>';
...
$res_memb = $xoopsDB->query(
    'SELECT u.name,u.uid,u.uname,ug.admin_flags ' . 'FROM ' . $xoopsDB->prefix('users') . ' u,' . $xoopsDB->prefix('xf_user_group') . ' ug ' . 'WHERE u.uid=ug.user_id ' . "AND ug.group_id='$group_id'"
);

$content = "<TABLE WIDTH='100%' BORDER='0'>";

while (false !== ($row_memb = $xoopsDB->fetchArray($res_memb))) {
    if ($perm->isAdmin()) {
        if (mb_stristr($row_memb['admin_flags'], 'A')) {
...
if ($perm->isAdmin()) {
    ...
    $content .= "<HR NoShade SIZE='1'>" . "<FORM NAME='adduserform' ACTION='userpermedit.php?group_id=" . $group->getID() . "' METHOD='POST'>" . "<INPUT TYPE='hidden' NAME='func' VALUE='adduser'>" . "<TABLE WIDTH='100%' BORDER='0'>" . "<TR><TD ALIGN='CENTER' ";

    //the user is an admin, we already know that so these checks are unnessesary

    /*$moduleHandler = xoops_getHandler('module');
    $membermodule = $moduleHandler->getByDirname('xoopsmembers');
    $modpermHandler = xoops_getHandler('groupperm');
    if ( $membermodule && $modpermHandler->checkRight("module", $membermodule->mid(), $xoopsUser->getGroups()))
    {*/

    $content .= "COLSPAN='3'>"
                . _XF_PRJ_HOWTOADDUSER
                . '</TD></TR>'
                . "<TR><TD ALIGN='RIGHT' WIDTH='50%'><B>"
                . _XF_PRJ_USERNAME
                . ':&nbsp;&nbsp;</B>'
                . "</TD><TD ALIGN='CENTER'><INPUT TYPE='TEXT' NAME='form_unix_name' SIZE='10' VALUE=''>"
                . "</TD><TD ALIGN='LEFT' WIDTH='50%'>&nbsp;&nbsp;<a href=\"#\" onClick=\"window.open('"
                . XOOPS_URL
                . "/modules/xoopsmembers/?userlookup=yes&isprj=yes','userlookup','status,scrollbars,height=480,width=800');return false\">User Lookup</a>"
                . '</TD></TR>'
                . "<TR><TD COLSPAN='3'";
    ...
    $content .= " ALIGN='CENTER'><INPUT TYPE='SUBMIT' NAME='submit' VALUE='" . _XF_PRJ_ADDUSER . "'></TD></TR></FORM>" . '</TABLE>'
                ...
                . "<HR NoShade SIZE='1'>" . "<div align='center'>" . "[ <A href='" . XOOPS_URL . '/modules/xfmod/project/admin/userperms.php?group_id=' . $group->getID() . "'>" . _XF_PRJ_EDITMEMBERPERMS . '</A> ]' . '</div>';
}
...
if ($group->isProject()) {
    ...
    while (false !== ($row = $xoopsDB->fetchArray($r))) {
        $content .= "<a href='" . $_SERVER['PHP_SELF'] . "?func=rmcvsmon&group_id=$group_id&notifyemail=" . urlencode($row['email']) . "'>" . "<img src='" . XOOPS_URL . "/modules/xfmod/images/ic/trash.png' alt='remove cvs monitor'>" . '</a> ' . $row['email'] . '<br>';
    }
...
$sql = 'SELECT p.group_id,p.unix_group_name FROM ' . $xoopsDB->prefix('xf_groups') . ' AS p, ' . $xoopsDB->prefix('xf_trove_group_link') . ' AS c' . " WHERE c.group_id=$group_id" . ' AND p.group_id=c.trove_cat_id';

$res_memb = $xoopsDB->query($sql);
while (false !== ($row = $xoopsDB->fetchArray($res_memb))) {
    if ($perm->isAdmin()) {
        $button = "<input type='image' name='delete' src='" . XOOPS_URL . "/modules/xfmod/images/ic/trash.png' height='16' width='16' border='0' alt='delete'>";
    } else {
        $button = '&nbsp;';
    }
    ...
    $content .= '<form action='
                . XOOPS_URL
                . "/modules/xfmod/project/admin/?group_id=$group_id "
                . " method='post'"
                ...
                . $row['group_id']
                . "'>"
                . "<input type='hidden' name='comm_name' value='"
                . $row['unix_group_name']
                . "'>"
                . "<tr><td align='middle'>"
                . $button
...
$content = '<BR>'
           . "<A HREF='"
           . XOOPS_URL
           . '/modules/xfmod/tracker/admin/?group_id='
           . $group->getID()
           . "'>"
           . _XF_PRJ_TRACKERADMIN
           . '</A><BR>'
           . "<A HREF='"
           . XOOPS_URL
           . '/modules/xfmod/docman/admin/?group_id='
           . $group->getID()
           . "'>"
           . _XF_PRJ_DOCMANAGERADMIN
           . '</A><BR>'
           . "<A HREF='"
           . XOOPS_URL
           . '/modules/xfmod/news/admin/?group_id='
           . $group->getID()
           . "'>"
           . _XF_PRJ_NEWSADMIN
           . '</A><BR>'
           . "<A HREF='"
           . XOOPS_URL
           . '/modules/xfmod/pm/admin/?group_id='
           . $group->getID()
           . "'>"
           . _XF_PRJ_TASKADMIN
           . '</A><BR>'
           . "<A HREF='"
           . XOOPS_URL
           . '/modules/xfmod/forum/admin/?group_id='
           . $group->getID()
           . "'>"
           . _XF_PRJ_FORUMADMIN
           . '</A><BR>';
...
$content = '&nbsp;<BR>'
           ...
           . $group_id
           . "'><B>"
           . _XF_PRJ_RELEASEEDITFILERELEASES
           . '</B></A> ]'
           . '</CENTER>'
           . '<HR>'
           . '<B>'
           ...
           . "<A href='"
           . XOOPS_URL
           . "/modules/xfmod/help/projects.php#downloading_projects'><i>"
           . _XF_PRJ_WHATISTHIS
           . '</i></A>'
           . '<P>';

$res_module = $xoopsDB->query('SELECT * FROM ' . $xoopsDB->prefix('xf_frs_package') . " WHERE group_id='$group_id'");
while (false !== ($row_module = $xoopsDB->fetchArray($res_module))) {
