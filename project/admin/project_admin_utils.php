...
function frs_show_release_popup($group_id, $name = 'release_id', $checked_val = 'xzxz')
{
    ...
    */ global $FRS_RELEASE_RES, $xoopsDB;
    ...
    if (!isset($FRS_RELEASE_RES)) {
        $FRS_RELEASE_RES = $xoopsDB->query(
                'SELECT r.release_id,r.name ' . 'FROM ' . $xoopsDB->prefix('xf_frs_release') . ' r,' . $xoopsDB->prefix('xf_frs_package') . ' p ' . "WHERE p.group_id='$group_id' " . 'AND r.package_id=p.package_id'
            );
...
function frs_show_package_popup($group_id, $name = 'package_id', $checked_val = 'xzxz')
{
    ...
    */ global $FRS_PACKAGE_RES, $xoopsDB;
    ...
    if (!isset($FRS_PACKAGE_RES)) {
        $FRS_PACKAGE_RES = $xoopsDB->query(
                'SELECT package_id,name ' . 'FROM ' . $xoopsDB->prefix('xf_frs_package') . ' ' . "WHERE group_id='$group_id'"
            );
...
function group_get_history($group_id = false)
{
    ...
    $sql = 'SELECT gh.field_name,gh.old_value,gh.date,u.uname ' . 'FROM ' . $xoopsDB->prefix('xf_group_history') . ' gh,' . $xoopsDB->prefix('users') . ' u ' . 'WHERE gh.mod_by=u.uid ' . "AND group_id='$group_id' ORDER BY gh.date DESC";
    ...
    if ($rows > 0) {
	  ...
	  <tr class="bg3" align="left">
        ...
        for ($i = 0; $i < $rows; $i++) {
            ...
            $content .= '</TD>' . '<TD>' . date($sys_datefmt, unofficial_getDBResult($result, $i, 'date')) . '</TD>' . '<TD>' . unofficial_getDBResult($result, $i, 'uname') . '</TD>' . '</TR>';
        }
