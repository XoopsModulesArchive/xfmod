...
$GLOBALS['xoopsOption']['template_main'] = 'project/admin/xfmod_editgroupinfo.html';
...
$xoopsTpl->assign('group_desc', $ts->htmlSpecialChars($group->getDescription()));
...
*/
$use[] = ['use_mail', 'Use Mailing Lists', c($group->usesMail())];
$use[] = ['use_survey', _XF_PRJ_USESURVEYS, c($group->usesSurvey())];
$use[] = ['use_forum', _XF_PRJ_USEFORUMS, c($group->usesForum())];
if ($group->isProject()) {
    $use[] = ['use_pm', _LOCAL_XF_PRJ_USEPROJECTMAN, c($group->usesPM())];

    $use[] = ['use_pm_depend', _XF_PRJ_USETASKDEP, c($group->usesPMDependencies())];

    //}else{

    $use[] = ['use_faq', 'Use FAQs', c($group->usesFAQ())];
}
$use[] = ['use_news', _XF_PRJ_USENEWS, c($group->usesNews())];
$use[] = ['use_docman', _XF_PRJ_USEDOCMGR, c($group->usesDocman())];
$use[] = ['use_sample', _XF_PRJ_USESAMPLEMGR, c($group->usesSamples())];
$use[] = ['use_tracker', _XF_PRJ_USETRACKER, c($group->usesTracker())];
