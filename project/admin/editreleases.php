...
$GLOBALS['xoopsOption']['template_main'] = 'project/admin/xfmod_editreleases.html';
...
if (isset($step3) && $step3) {
    ...
    if ('Delete File' == $step3 && 'Delete File' == $submit && $im_sure) {
    ...
    } elseif ('Delete File' == $step3 && 'Update File' == $submit) {
    ...
    <tr class='bg3' align='left'>
        ...
        for ($i = 0; $i < $rows; $i++) {
            $content .= "<tr class='"
                        ...
                        . '<td>'
                        . "<font size='-1'>"
                        . $ts->htmlSpecialChars(unofficial_getDBResult($res, $i, 'release_name'))
                        ...
                        . $group_id
                        . "'>"
                        . _XF_PRJ_EDITTHISRELEASE
                        . '</a> ]'
                        . '</font>'
                        . '</td>'
                        . '<td>'
                        . "<font size='-1'>"
                        . $ts->htmlSpecialChars(unofficial_getDBResult($res, $i, 'package_name'))
                        . "[ <a href='editpackages.php?group_id="
                        . $group_id
                        . "'>"
                        . _XF_PRJ_EDITTHISPACKAGE
                        . '</a> ]'
                        . '</font>'
                        . '</td>'
                        . '<td>'
                        . "<font size='-1'>"
                        . unofficial_getDBResult($res, $i, 'status_name')
                        . '</font>'
                        . '</td>'
                        . '</tr>'
                        . '</form>';
...
<tr>
        ...
        <td><input type='checkbox' name='status_id' value='3' " . ((1 == unofficial_getDBResult($result, 0, 'status_id')) ? '' : 'checked') . '> Private
        </td>
</tr>
<tr>
...
<tr>
        <td COLSPAN='2'>
                ...
                <textarea name='release_notes' rows='10' cols='60' wrap='soft'>" . $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'notes')) . '</textarea>
        </td>
</TR>
<TR>
        <td COLSPAN=2>
                <b>' . _XF_PRJ_PASTELOG . ":</b><br>
                <textarea name='release_changes' rows='10' cols='60' wrap='soft'>" . $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'changes')) . '</textarea>
        </td>
</tr>
<TR>
        <td COLSPAN=2>
                <b>' . _XF_PRJ_PASTEDEPENDENCIES . ":</b><br>
                <textarea name='release_dependencies' rows='10' cols='60' wrap='soft'>" . $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'dependencies')) . "</textarea>
        </td>
</tr>
<TR>
        <TD COLSPAN=2>
...
<B>' . _XF_PRJ_STEP3 . '</B><p>';
    ...
    <tr class='bg1' align='left'>
            ...
            for ($x = 0; $x < $rows; $x++) {
                ...
                if (mb_strstr(unofficial_getDBResult($res, $x, 'file_url'), '://')) {
                    $content .= "
                ...
                if (preg_match("/.*\.rpm$/", unofficial_getDBResult($res, $x, 'filename')) && !preg_match("/.*\.src\.rpm$/", unofficial_getDBResult($res, $x, 'filename'))) {
                    ...
                    [$mycount] = $xoopsDB->fetchRow($myresult);
