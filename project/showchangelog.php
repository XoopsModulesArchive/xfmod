...
$GLOBALS['xoopsOption']['template_main'] = 'project/xfmod_showchangelog.html';
...
$query = 'SELECT r.changes, r.preformatted, p.name ' . ' FROM ' . $xoopsDB->prefix('xf_frs_release') . ' r LEFT JOIN ' . $xoopsDB->prefix('xf_frs_package') . ' p ON r.package_id=p.package_id ' . " WHERE  p.group_id='$group_id' " . ' ORDER BY p.name';
...
if (!$result || $xoopsDB->getRowsNum($result) < 1) {
    ...
    */
    ...
    if (unofficial_getDBResult($result, 0, 'preformatted')) {
        ...
        $xoopsTpl->assign('change_content', '<pre>' . $ts->displayTarea($completeChangeLog) . '</pre>');
    } else {
        $xoopsTpl->assign('changelog_title', _XF_PRJ_CHANGELOG);

        $xoopsTpl->assign('changelog_content', $ts->displayTarea($completeChangeLog));
    }
}
