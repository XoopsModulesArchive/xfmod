...
$GLOBALS['xoopsOption']['template_main'] = 'project/xfmod_shownotes.html';
...
$result = $xoopsDB->query(
    'SELECT r.notes,r.changes,r.dependencies,r.preformatted,r.name,p.group_id ' . 'FROM ' . $xoopsDB->prefix('xf_frs_release') . ' r,' . $xoopsDB->prefix('xf_frs_package') . ' p ' . 'WHERE r.package_id=p.package_id ' . "AND r.release_id='$release_id'"
);
...
if (!$result || $xoopsDB->getRowsNum($result) < 1) {
    ...
    $content = "<h2 style='text-align:left;'>" . _XF_PRJ_RELEASENAME . ": <A HREF='showfiles.php?group_id=" . $group_id . "'>" . $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'name')) . '</A></H3><P>';
    ...
    */
    ...
    if (unofficial_getDBResult($result, 0, 'preformatted')) {
        $notes = trim($ts->displayTarea(unofficial_getDBResult($result, 0, 'notes')));
        ...
        $changelog = trim($ts->displayTarea(unofficial_getDBResult($result, 0, 'changes')));
        ...
        $dependencies = trim($ts->displayTarea(unofficial_getDBResult($result, 0, 'dependencies')));
