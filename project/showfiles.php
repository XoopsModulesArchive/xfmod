...
$GLOBALS['xoopsOption']['template_main'] = 'project/xfmod_showfiles.html';
...
for ($p = 0; $p < $num_packages; $p++) {
    ...
    $private_package = (3 == $package['status_id']);

    $content .= "\n<tr><td colspan='4'><h3>" . $ts->htmlSpecialChars($package['name']) . ' - (' . $ts->htmlSpecialChars($package['status']) . ")</h3></td></tr>\n";

    // Get the releases of the package.

    $sql = 'SELECT *' . ' FROM ' . $xoopsDB->prefix('xf_frs_release') . ' WHERE package_id=' . $package['package_id'] . " AND status_id<=$rights" . ' ORDER BY release_date DESC, name ASC';
    ...
    if (!$rs_releases || $num_releases < 1) {
        ...
        for ($r = 0; $r < $num_releases; $r++) {
            ...
            $private_release = (3 == $release['status_id']);
            ...
            $content .= "<tr class='bg3'>\n"
                        . "\t<td colspan='3'"
                        . ((isset($highlight_name) && $highlight_name) ? " style='border-top:2px solid red;border-left:2px solid red'" : '')
                        . '>'
                        //."&nbsp;<b><a $highlight_name href='shownotes.php?group_id=$group_id&release_id=".$release['release_id']."'>"
                        . "&nbsp;<b><a href='shownotes.php?group_id=$group_id&release_id="
                        . $release['release_id']
                        . "'>"
                        . (mb_strlen($release['name']) ? $ts->htmlSpecialChars($release['name']) : ('<i>(' . _XF_PRJ_VIEWNOTES . ')</i>'))
                        . "</a></b></td>\n"
                        . "\t<td align='right'"
                        . ((isset($highlight_name) && $highlight_name) ? " style='border-top:2px solid red;border-right:2px solid red'" : '')
                        . '><b>'
            ...
            $sql = 'SELECT filename, file_size, file_url, release_id, '
                   . $xoopsDB->prefix('xf_frs_file')
                   . '.file_id, release_time, downloads, status'
                   . ' FROM '
                   . $xoopsDB->prefix('xf_frs_file')
                   . ' LEFT JOIN '
                   . $xoopsDB->prefix('xf_webservice_publish')
                   . ' USING (file_id) '
                   . ' LEFT JOIN '
            ...
            if (!$rs_files || $num_files < 1) {
                ...
                for ($f = 0; $f < $num_files; $f++) {
                    ...
                    $content .= "&nbsp;&nbsp;&nbsp;&nbsp;</td>\n"
                                . "\t<td align='right'"
                                . (($highlight_name && $f + 1 == $num_files) ? " style='border-bottom:2px solid red'" : '')
                                . '>'
                                . ($file['file_size'] ? readableSize($file['file_size']) : '-')
                                . "&nbsp;&nbsp;&nbsp;&nbsp;</td>\n"
                                . "\t<td align='center'"
                                . (($highlight_name && $f + 1 == $num_files) ? " style='border-bottom:2px solid red'" : '')
                                . '>'
                                . date('Y-m-d', $file['release_time'])
                                . "&nbsp;&nbsp;&nbsp;&nbsp;</td>\n"
                                . "\t<td align='right'"
                                . ($highlight_name ? " style='border-right:2px solid red" . ($f + 1 == $num_files ? ";border-bottom:2px solid red'" : "'") : '')
                                . '>'
                                . ($file['downloads'] ? number_format($file['downloads'], 0) : '0')
                                . '&nbsp;'
                                . _XF_PRJ_DL
                                . "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>\n"
                                . "</tr>\n";
...
$message = ($usesRedCarpet ? "Indicates that the file is available from <a style='text-decoration:underline' href='/modules/xfmod/help/projects.php#red_carpet'>Forge's Red Carpet server</a>." : "No files are available from <a style='text-decoration:underline' href='$url2'>Forge's Red Carpet server</a> for this project.");
