...
//require_once XOOPS_ROOT_PATH."/class/nxoopsLDAP.php";
require_once 'ndk.php';
...
if ('save' == $op) {
    ...
    if (!('dirxml' == $group_name
          || 'dirxmllin' == $group_name
          || 'dirxmlsol' == $group_name
          || 'cldap' == $group_name
          || 'cldaplin' == $group_name
          || 'cldapsol' == $group_name
          || 'cldapaix' == $group_name
          || 'cldaphpux' == $group_name
          || 'jldap' == $group_name
          || 'jldapunx' == $group_name
          || 'jvm117b' == $group_name
          || 'jvm13' == $group_name
        ...
        while (list($key, $val) = each($ndk)) {
            if ($group_name == $val->shortname) {
                ...
                $pres = $xoopsDB->query(
                    'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_frs_package') . ' ' . "WHERE group_id='" . $group_id . "' AND name LIKE '%NDK%'"
                );
                ...
                for ($i = 0; $i < $prows; $i++) {
                    ...
                    while (list($key2, $val2) = each($tempdown)) {
                        ...
                        if (((mb_strpos($val2->getName(), '_al.') || mb_strpos($val2->getName(), '_all.')) && mb_strpos($package_name, 'Complete'))
                            || (mb_strpos($val2->getName(), '_doc.') && mb_strpos($package_name, 'Documentation'))
                            || (mb_strpos($val2->getName(), '_sample.') && mb_strpos($package_name, 'Sample'))
                            || (!((mb_strpos($val2->getName(), '_al.') || mb_strpos($val2->getName(), '_all.'))) && !mb_strpos($val2->getName(), '_doc.') && !mb_strpos($val2->getName(), '_sample.') && mb_strpos($package_name, 'Binaries'))) {
                            if ('Y' == $val2->getUpdate()) {
                                // if Y, then update the file path of the last file like this to /2004/jun

                                $rres = $xoopsDB->query(
                                    'SELECT * FROM ' . $xoopsDB->prefix('xf_frs_release') . ' ' . "WHERE package_id='" . $package_id . "' ORDER BY 'release_date' DESC"
                                );
                                ...
                                if (0 != $rrows) {
                                    ...
                                    $fres = $xoopsDB->query(
                                        'SELECT * FROM ' . $xoopsDB->prefix('xf_frs_file') . ' ' . "WHERE filename LIKE '%.exe' AND release_id='" . $rel_id . "'"
                                    );
                                    ...
                                    for ($k = 0; $k < $frows; $k++) {
                                        ...
                                        $file_url = substr_replace($val2->getPath(), '2004/jun/', mb_strrpos($val2->getPath(), 'filename=') + 9, 0);
                            ...
                            $date_list = preg_split('-', $val2->getModified(), 3);
                ...
                $query = 'DELETE FROM ' . $xoopsDB->prefix('xf_doc_data') . ' ' . "WHERE title LIKE '%download%'";

                $xoopsDB->queryF($query);

                $query = 'DELETE FROM ' . $xoopsDB->prefix('xf_sample_data') . ' ' . "WHERE title LIKE '%download%'";
    ...
    } elseif ('cldap' == $group_name || 'cldaplin' == $group_name || 'cldapsol' == $group_name || 'cldapaix' == $group_name || 'cldaphpux' == $group_name) {
        // needed only for unix_name check in frs.class
        ...
        while (list($key, $val) = each($ndk)) {
            if ($group_name == $val->shortname) {
                ...
                $pres = $xoopsDB->query(
                    'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_frs_package') . ' ' . "WHERE group_id='" . $cldap . "' AND name LIKE '%NDK%'"
                );
                ...
                for ($i = 0; $i < $prows; $i++) {
                    ...
                    $relres = $xoopsDB->query(
                        'SELECT * FROM ' . $xoopsDB->prefix('xf_frs_release') . ' ' . "WHERE name = '" . $release_name . "' AND package_id='" . $package_id . "'"
                    );
                    ...
                    while (list($key2, $val2) = each($tempdown)) {
                        ...
                        if ((mb_strpos($val2->getName(), 'lin_all.') && mb_strpos($package_name, 'Complete Download - Linux'))
                            || (mb_strpos($val2->getName(), 'lin_doc.') && mb_strpos($package_name, 'Documentation - Linux'))
                            || (mb_strpos($val2->getName(), 'lin_sample.') && mb_strpos($package_name, 'Sample Code - Linux'))
                            || (mb_strpos($val2->getName(), 'sol_all.') && mb_strpos($package_name, 'Complete Download - Solaris'))
                            || (mb_strpos($val2->getName(), 'sol_doc.') && mb_strpos($package_name, 'Documentation - Solaris'))
                            || (mb_strpos($val2->getName(), 'sol_sample.') && mb_strpos($package_name, 'Sample Code - Solaris'))
                            || (mb_strpos($val2->getName(), 'hpux_all.') && mb_strpos($package_name, 'Complete Download - HP'))
                            || (mb_strpos($val2->getName(), 'hpux_doc.') && mb_strpos($package_name, 'Documentation - HP'))
                            || (mb_strpos($val2->getName(), 'hpux_sample.') && mb_strpos($package_name, 'Sample Code - HP'))
                            || (mb_strpos($val2->getName(), 'aix_all.') && mb_strpos($package_name, 'Complete Download - AIX'))
                            || (mb_strpos($val2->getName(), 'aix_doc.') && mb_strpos($package_name, 'Documentation - AIX'))
                            || (mb_strpos($val2->getName(), 'aix_sample.') && mb_strpos($package_name, 'Sample Code - AIX'))
                            || (mb_strpos($val2->getName(), '_all.') && !mb_strpos($val2->getName(), 'lin') && !mb_strpos($val2->getName(), 'sol') && !mb_strpos($val2->getName(), 'hpux') && !mb_strpos($val2->getName(), 'aix') && mb_strpos($package_name, 'Complete Download - NetWare'))
                            || (mb_strpos($val2->getName(), '_doc.') && !mb_strpos($val2->getName(), 'lin') && !mb_strpos($val2->getName(), 'sol') && !mb_strpos($val2->getName(), 'hpux') && !mb_strpos($val2->getName(), 'aix') && mb_strpos($package_name, 'Documentation - NetWare'))
                            || (mb_strpos($val2->getName(), '_sample.') && !mb_strpos($val2->getName(), 'lin') && !mb_strpos($val2->getName(), 'sol') && !mb_strpos($val2->getName(), 'hpux') && !mb_strpos($val2->getName(), 'aix') && mb_strpos($package_name, 'Sample Code - NetWare'))
                            || (!mb_strpos($val2->getName(), '_all.') && !mb_strpos($val2->getName(), '_doc.') && !mb_strpos($val2->getName(), '_sample.')
                                && ((mb_strpos($val2->getName(), 'lin') && mb_strpos($package_name, 'Binaries - Linux'))
                                    || (mb_strpos($val2->getName(), 'sol')
                                        && mb_strpos(
                                            $package_name,
                                            'Binaries - Solaris'
                                        ))
                                    || (mb_strpos($val2->getName(), 'hpux') && mb_strpos($package_name, 'Binaries - HP'))
                                    || (mb_strpos($val2->getName(), 'aix') && mb_strpos($package_name, 'Binaries - AIX'))
                                    || (!mb_strpos($val2->getName(), 'lin')
                                        && !mb_strpos($val2->getName(), 'sol')
                                        && !mb_strpos($val2->getName(), 'hupx')
                                        && !mb_strpos($val2->getName(), 'aix')
                                        && mb_strpos($package_name, 'Binaries - NetWare'))))) {
                            if ('Y' == $val2->getUpdate()) {
                                // if Y, then update the file path of the last file like this to /2004/jun
    ...
    } elseif ('jvm117b' == $group_name || 'jvm13' == $group_name || 'jvm141' == $group_name || 'jvm142' == $group_name) {
        // needed only for unix_name check in frs.class
        ...
        while (list($key, $val) = each($ndk)) {
            if ($group_name == $val->shortname) {
                ...
                $pres = $xoopsDB->query(
                    'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_frs_package') . ' ' . "WHERE group_id='" . $jvm . "' AND name LIKE '%NDK%'"
                );
    ...
    } elseif ('dirxml' == $group_name || 'dirxmllin' == $group_name || 'dirxmlsol' == $group_name) {
        // needed only for unix_name check in frs.class
        ...
        while (list($key, $val) = each($ndk)) {
            if ($group_name == $val->shortname) {
                ...
                $pres = $xoopsDB->query(
                    'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_frs_package') . ' ' . "WHERE group_id='" . $dirxml . "' AND name LIKE '%NDK%'"
                );
                ...
                for ($i = 0; $i < $prows; $i++) {
                    ...
                    while (list($key2, $val2) = each($tempdown)) {
                        ...
                        if (((mb_strpos($val2->getName(), '_al.') || mb_strpos($val2->getName(), '_all.')) && mb_strpos($package_name, 'Complete'))
                            ...
                            if (mb_strstr($val2->getName(), 'lin')) {
                                $file_name = $val2->getName() . ' (Linux)';
                            } elseif (mb_strstr($val2->getName(), 'sol')) {
    ...
    } elseif ('jldap' == $group_name || 'jldapunx' == $group_name) {
        // needed only for unix_name check in frs.class
        ...
        while (list($key, $val) = each($ndk)) {
            if ($group_name == $val->shortname) {
                ...
                $pres = $xoopsDB->query(
                    'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_frs_package') . ' ' . "WHERE group_id='" . $jldap . "' AND name LIKE '%NDK%'"
                );
                ...
                for ($i = 0; $i < $prows; $i++) {
                    ...
                    while (list($key2, $val2) = each($tempdown)) {
                        ...
                        if (((mb_strpos($val2->getName(), 'unx_all.') && mb_strpos($package_name, 'Complete Download - UNIX')) || (mb_strpos($val2->getName(), '_all.') && !mb_strpos($val2->getName(), 'unx') && mb_strpos($package_name, 'Complete Download - NetWare')))
                            || ((mb_strpos($val2->getName(), 'unx_doc.') && mb_strpos($package_name, 'Documentation - UNIX')) || (mb_strpos($val2->getName(), '_doc.') && !mb_strpos($val2->getName(), 'unx') && mb_strpos($package_name, 'Documentation - NetWare')))
                            || ((mb_strpos($val2->getName(), 'unx_sample.') && mb_strpos($package_name, 'Sample Code - UNIX')) || (mb_strpos($val2->getName(), '_sample.') && !mb_strpos($val2->getName(), 'unx') && mb_strpos($package_name, 'Sample Code - NetWare')))
                            || ((!mb_strpos($val2->getName(), 'unx_all.') && !mb_strpos($val2->getName(), 'unx_doc.') && !mb_strpos($val2->getName(), 'unx_sample.') && mb_strpos($val2->getName(), 'unx') && mb_strpos($package_name, 'Binaries - UNIX'))
                                || (!mb_strpos($val2->getName(), '_all.')
                                    && !mb_strpos(
                                        $val2->getName(),
                                        '_doc.'
                                    )
                                    && !mb_strpos($val2->getName(), '_sample.')
                                    && !mb_strpos($val2->getName(), 'unx')
                                    && mb_strpos($package_name, 'Binaries - NetWare')))) {
                            if ('Y' == $val2->getUpdate()) {
                                // if Y, then update the file path of the last file like this to /2004/jun
    ...
    } elseif ('odbc' == $group_name || 'odbcrw' == $group_name) {
        // needed only for unix_name check in frs.class
        ...
        while (list($key, $val) = each($ndk)) {
            if ($group_name == $val->shortname) {
                ...
                $pres = $xoopsDB->query(
                    'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_frs_package') . ' ' . "WHERE group_id='" . $odbc . "' AND name LIKE '%NDK%'"
                );
    ...
    } elseif ('php' == $group_name || 'php2' == $group_name) {
        // needed only for unix_name check in frs.class
        ...
        while (list($key, $val) = each($ndk)) {
            if ($group_name == $val->shortname) {
                ...
                $pres = $xoopsDB->query(
                    'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xf_frs_package') . ' ' . "WHERE group_id='" . $php . "' AND name LIKE '%NDK%'"
                );
                        ...
                        if (((mb_strpos($val2->getName(), '_al.') || mb_strpos($val2->getName(), '_all.')) && mb_strpos($package_name, 'Complete'))
                            ...
                            if ('Y' == $val2->getUpdate()) {
                                ...
                                if (0 != $rrows) {
                                    ...
                                    for ($k = 0; $k < $frows; $k++) {
                                        if (rtrim(mb_substr(unofficial_getDBResult($fres, $k, 'filename'), 0, mb_strpos(unofficial_getDBResult($fres, $k, 'filename'), '('))) == $val2->getName()) {
                                            $file_id = unofficial_getDBResult($fres, $k, 'file_id');
                            ...
                            if (mb_strstr($val2->getName(), 'php2')) {
    ...
    $unix_name = mb_strtolower($_POST['group_name']);
    ...
    if ($xoopsDB->getRowsNum(
        $xoopsDB->query(
                'SELECT group_id FROM ' . $xoopsDB->prefix('xf_groups') . " WHERE unix_group_name='$unix_name'"
            )
    ) > 0) {
    ...
    while (list($key, $val) = each($ndk)) {
        ...
        if ($rows > 0 && !('dirxml' == $val->getShortName() || 'cldap' == $val->getShortName() || 'jldap' == $val->getShortName())) {
            ...
            echo '<h2>'
                 . $val->getName()
                 . "</h2><font color='red'>update existing project for Release &nbsp;&nbsp;<input type='text' name='release_name' value='June 2004' size='10'> <input type='text' name='release_date' value='2004-06-09' size='10'></font><input type='hidden' name='op' value='save'> <input type='submit' value='add "
                 . $val->getShortName()
                 . "'>";
            ...
            if ('dirxml' == $val->getShortName() || 'dirxmllin' == $val->getShortName() || 'dirxmlsol' == $val->getShortName() || 'cldap' == $val->getShortName() || 'cldaplin' == $val->getShortName() || 'cldapsol' == $val->getShortName() || 'cldapaix' == $val->getShortName()
                || 'cldaphpux' == $val->getShortName()
                || 'jldap' == $val->getShortName()
                || 'jldapunx' == $val->getShortName()
                || 'jvm117b' == $val->getShortName()
                || 'jvm13' == $val->getShortName()
                || 'jvm141' == $val->getShortName()
                || 'jvm142' == $val->getShortName()
                || 'odbc' == $val->getShortName()
                || 'odbcrw' == $val->getShortName()
                || 'php' == $val->getShortName()
                || 'php2' == $val->getShortName()) {
                echo "<form method='POST' action=''><input type='hidden' name='group_name' value='" . $val->getShortName() . "'>";

                echo '<h2>'
                     . $val->getName()
                     . "</h2><font color='red'>Special Case &nbsp;&nbsp;<input type='text' name='release_name' value='June 2004' size='10'> <input type='text' name='release_date' value='2004-06-09' size='10'></font><input type='hidden' name='op' value='save'> <input type='submit' value='add "
                     . $val->getShortName()
                     . "'>";
...
function parse()
{
    ...
    xml_set_elementHandler($xml_parser, 'start_tag', 'end_tag');

    xml_set_character_dataHandler($xml_parser, 'tag_contents');
...
function start_tag($parser, $name, $attribs)
{
        ...
        if (is_array($attribs)) {
            while (list($key, $val) = each($attribs)) {
                if ('SHORTNAME' == $key) {
                    ...
                    if ('ldapcsharp' == $val || 'psqlsdk' == $val || 'apache' == $val || 'mysql' == $val || 'rsync' == $val || 'postgres' == $val || 'cwpdkc' == $val || 'uddiaix' == $val || 'uddisol' == $val || 'uddiwin' == $val || 'uddinw' == $val || 'uddilin' == $val || 'gwsdk' == $val
                        || 'bns-index' == $val
                        || 'dirxml-index' == $val
                        || 'jbrokermq' == $val
                        || 'jbrokertm' == $val
                    ...
                    $res = $xoopsDB->query(
                            'SELECT group_id ' . 'FROM ' . $xoopsDB->prefix('xf_groups') . ' ' . "WHERE unix_group_name='" . $val . "'"
                        );
    ...
    if ('ATTRIBUTE' == $name) {
        if (is_array($attribs)) {
            while (list($key, $val) = each($attribs)) {
                if ('RECORDTYPE' == $key && 'Name' == $val) {
                    //echo "<h4>Name</h4>";

                    $tempndk->attribute = 'Name';
                } elseif ('RECORDTYPE' == $key && 'Description' == $val) {
                    //echo "<h4>Description</h4>";

                    $tempndk->attribute = 'Description';
                } elseif ('RECORDTYPE' == $key && 'Whats New' == $val) {
                    //echo "<h4>Whats New</h4>";

                    $tempndk->attribute = 'Whats New';
                } elseif ('RECORDTYPE' == $key && 'Whats New Archive' == $val) {
                    //echo "<h4>Whats New Archive</h4>";

                    $tempndk->attribute = 'Whats New Archive';
                } elseif ('RECORDTYPE' == $key && 'Dependencies' == $val) {
                    //echo "<h4>Dependencies</h4>";

                    $tempndk->attribute = 'Dependencies';
                } elseif ('RECORDTYPE' == $key && 'Support Status' == $val) {
        ...
        if (is_array($attribs)) {
            ...
            while (list($key, $val) = each($attribs)) {
                if ('NAME' == $key) {
                    ...
                    if (mb_strpos($val, '_al.') || mb_strpos($val, '_all.')) {
                        //echo "All : ";

                        $tempdownload->type = 'Complete';

                        if (mb_strstr($val, '.exe') || mb_strstr($val, 'dirxml')) {
...
function tag_contents($parser, $data)
{
    ...
    if ('Name' == $tempndk->attribute) {
        $tempndk->setName($data);
    } elseif ('Description' == $tempndk->attribute) {
        $tempndk->setDescription($data);
    } elseif ('Dependencies' == $tempndk->attribute) {
        $tempndk->setDependencies($data);
    } elseif ('Whats New' == $tempndk->attribute) {
        $tempndk->setWhatsNew($data);
    } elseif ('Whats New Archive' == $tempndk->attribute) {
        $tempndk->setWhatsNewArchive($data);
    } elseif ('Support Status' == $tempndk->attribute) {
