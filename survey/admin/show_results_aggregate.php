...
$GLOBALS['xoopsOption']['template_main'] = 'survey/admin/xfmod_show_results_aggregates.html';
...
$content = '<H4>' . $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'survey_title')) . '</H4><P>';
...
for ($i = 0; $i < $count; $i++) {
    ...
    if ('1' == $question_type) {
        ...
        if ($response_count > 0) {
            ...
            //$content .= GraphResult($result2,$ts->htmlSpecialChars(unofficial_getDBResult($result, 0, "question")));
        }// end if (responses to survey question present)
    } elseif ('2' == $question_type) {
        // This is a text-area question.

        $content .= $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'question')) . "<BR>\n";
    ...
    //$content .= GraphIt($name_array,$value_array,$ts->htmlSpecialChars(unofficial_getDBResult($result, 0, "question")));
    } elseif ('4' == $question_type) {
        // This is a comment only.

        $content .= '&nbsp;<P><B>' . $ts->htmlSpecialChars(unofficial_getDBResult($result, 0, 'question')) . "</B>\n";
