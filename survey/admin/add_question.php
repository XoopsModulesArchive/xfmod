...
$GLOBALS['xoopsOption']['template_main'] = 'survey/admin/xfmod_add_question.html';
...
if ($post_changes) {
    ...
    $quest = $ts->addSlashes($question);

    if (mb_strlen($quest) < 1) {
        $xoopsForgeErrorHandler->addError('You must specify a question');
    } else {
        $sql = 'INSERT INTO ' . $xoopsDB->prefix('xf_survey_questions') . " (group_id,question,question_type) VALUES ('$group_id'," . "'$quest','$question_type')";
        ...
        if ($result) {
            ...
            $xoopsForgeErrorHandler->addError(
                'Failed adding question - ' . $xoopsDB->error()
            );
        }
    }
}
