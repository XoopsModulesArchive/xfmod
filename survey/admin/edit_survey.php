...
$GLOBALS['xoopsOption']['template_main'] = 'survey/admin/xfmod_edit_survey.html';
...
if ($post_changes) {
    $sql = 'UPDATE ' . $xoopsDB->prefix('xf_surveys') . ' ' . "SET survey_title='$survey_title'," . "survey_questions='$survey_questions'," . "is_active='$is_active' " . "WHERE survey_id='$survey_id' " . "AND group_id='$group_id'";

    $result = $xoopsDB->queryF($sql);

    if (!$result) {
        $xoopsForgeErrorHandler->addError(
            'Survey Update Failed - ' . $xoopsDB->error()
        );
    } else {
        $msg = 'Survey "' . $survey_title . '" has been updated. ID=' . $survey_id;

        redirect_header(
            XOOPS_URL . '/modules/xfmod/survey/admin/' . 'browse_surveys.php?group_id=' . $group_id . '&feedback=' . urlencode($msg),
            0,
            ''
        );
    }
...
$xoopsTpl->assign('survey_title', $ts->htmlSpecialChars($survey_title));
