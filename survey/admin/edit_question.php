...
$GLOBALS['xoopsOption']['template_main'] = 'survey/admin/xfmod_edit_question.html';
...
if ($post_changes) {
    $qtext = $ts->addSlashes($question);

    if (mb_strlen($qtext) < 1) {
        $xoopsForgeErrorHandler->addError('You must specify a question');

        $doingerror = true;
    } else {
        $sql = 'UPDATE ' . $xoopsDB->prefix('xf_survey_questions') . ' ' . "SET question='" . $ts->addSlashes($question) . "'," . "question_type='$question_type' " . "WHERE question_id='$question_id' " . "AND group_id='$group_id'";

        $result = $xoopsDB->queryF($sql);

        if (!$result) {
            $xoopsForgeErrorHandler->addError(
                'Update failed - ' . $xoopsDB->error()
            );
        } else {
            $updtxt = "Question $question_id updated";

            redirect_header(
                XOOPS_URL . '/modules/xfmod/survey/admin' . '/show_questions.php?group_id=' . $group_id . '&updtxt=' . $updtxt,
                0,
                ''
            );
        }
...
if (!$doingerror) {
    $sql = 'SELECT * FROM ' . $xoopsDB->prefix('xf_survey_questions') . ' ' . "WHERE question_id='$question_id' AND group_id='$group_id'";
...
$xoopsTpl->assign('question', $ts->htmlSpecialChars($question));
