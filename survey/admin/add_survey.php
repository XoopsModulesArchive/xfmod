...
require_once XOOPS_ROOT_PATH . '/modules/xfmod/survey/survey_common.php';
...
$GLOBALS['xoopsOption']['template_main'] = 'survey/admin/xfmod_add_survey.html';
...
if ($post_changes) {
    ...
    if (mb_strlen($survey_questions) < 1) {
    ...
    if ($isvalid) {
        $sql = 'INSERT INTO ' . $xoopsDB->prefix('xf_surveys') . ' (survey_title,group_id,survey_questions) ' . "VALUES ('$survey_title','$group_id','$survey_questions')";
        ...
        if ($result) {
            ...
            $xoopsForgeErrorHandler->addError(
                'Failed to add survey: ' . $xoopsDB->error()
            );
        }
    }
}
...
//require XOOPS_ROOT_PATH."modules/xfmod/survey/admin/quest_spec.php";
