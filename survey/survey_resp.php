...
$result = $xoopsDB->queryF(
    'DELETE FROM ' . $xoopsDB->prefix('xf_survey_responses') . ' ' . "WHERE survey_id='" . addslashes($survey_id) . "' " . "AND group_id='" . addslashes($group_id) . "' " . "AND user_id='" . $xoopsUser->getVar('uid') . "'"
);
...
for ($i = 0; $i < $count; $i++) {
    ...
    */
    ...
    $sql = 'INSERT INTO ' . $xoopsDB->prefix('xf_survey_responses') . ' (user_id,group_id,survey_id,question_id,response,date) ' . "VALUES ('" . $xoopsUser->getVar('uid') . "','" . addslashes($group_id) . "','" . addslashes($survey_id) . "','" . addslashes($quest_array[$i]) . "','" . addslashes(
        $$val
    ) . "','$now')";
